module speed_logical_operators_test
...
contains
    function test_equal_operator() result(tests)
        ...
        individual_tests(1) = it( &
                "is equal for the same speed", &
                DOUBLE_PRECISION_GENERATOR, &
                checkEqualWithSameNumber)
        individual_tests(2) = it( &
                "is not equal for different speeds", &
                DOUBLE_PRECISION_GENERATOR, &
                checkEqualWithDifferentNumbers)
        tests = describe("Speed_t with operator(==)", individual_tests)
    ...
    function test_equal_within() result(tests)
        ...
        individual_tests(1) = it( &
                "is equal for the same speed even for tiny tolerance", &
                DOUBLE_PRECISION_GENERATOR, &
        ...
        tests = describe("Speed_t%equal with tolerance", individual_tests)
    ...
    function test_greater_than_or_equal_operator() result(tests)
        ...
        tests = describe("Speed_t operator (>=)", &
                individual_tests)
    ...
    pure function checkEqualWithSameNumber(number) result(result_)
        ...
        type is (DoublePrecisionInput_t)
            the_speed = number%value_.unit.METERS_PER_SECOND
            result_ = assertThat( &
                    the_speed == the_speed, &
                    the_speed%toString() // " == " // the_speed%toString())
        class default
    ...
    pure function checkEqualWithDifferentNumbers(number) result(result_)
        ...
        select type (number)
        type is (DoublePrecisionInput_t)
            speed1 = number%value_.unit.METERS_PER_SECOND
            speed2 = (1.0d0 + number%value_).unit.METERS_PER_SECOND
            result_ = assertNot( &
                    speed1 == speed2, &
                    speed1%toString() // " == " // speed2%toString())
        class default
            result_ = fail("Expected to get a DoublePrecisionInput_t")
        end select
    ...
    pure function checkEqualWithinWithSameNumber(number) result(result_)
        ...
        type is (DoublePrecisionInput_t)
            ...
            result_ = assertThat( &
                    ...
                    "(" // the_speed%toString() // ")%equal(" &
                        // the_speed%toString() // ", within = " &
                        // tolerance%toString() // ")")
        class default
            result_ = fail("Expected to get a DoublePrecisionInput_t")
        end select
    ...
    pure function checkEqualWithinWithCloseNumbers(number) result(result_)
        ...
        type is (DoublePrecisionInput_t)
            speed1 = number%value_.unit.METERS_PER_SECOND
            speed2 = (number%value_ + 0.05d0).unit.METERS_PER_SECOND
            tolerance = 0.1d0.unit.METERS_PER_SECOND
            result_ = assertThat( &
                    speed1%equal(speed2, within = tolerance), &
                    "(" // speed1%toString() // ")%equal(" &
                        // speed2%toString() // ", within = " &
                        // tolerance%toString() // ")")
        class default
            result_ = fail("Expected to get a DoublePrecisionInput_t")
        end select
    ...
    pure function checkEqualWithinWithDifferentNumbers(number) result(result_)
        ...
        type is (DoublePrecisionInput_t)
            speed1 = number%value_.unit.METERS_PER_SECOND
            speed2 = (number%value_ + 0.2d0).unit.METERS_PER_SECOND
            tolerance = 0.1d0.unit.METERS_PER_SECOND
            result_ = assertNot( &
                    speed1%equal(speed2, within = tolerance), &
                    "(" // speed1%toString() // ")%equal(" &
    ...
    pure function checkGreaterThanOrEqualWithGreaterNumbers(number) result(result_)
        ...
        type is (DoublePrecisionInput_t)
            speed1 = number%value_.unit. METERS_PER_SECOND
            speed2 = (number%value_ - 1.0d0).unit.METERS_PER_SECOND
            result_ = assertThat( &
                    speed1 >= speed2, &
                    speed1%toString() // " >= " // speed2%toString())
        class default
    ...
    pure function checkGreaterThanOrEqualWithSameNumbers(number) result(result_)
        ...
        type is (DoublePrecisionInput_t)
            speed1 = number%value_.unit.METERS_PER_SECOND
            speed2 = number%value_.unit.METERS_PER_SECOND
            result_ = assertThat( &
                    speed1 >= speed2, &
                    speed1%toString() // " >= " // speed2%toString())
        class default
...
end module speed_logical_operators_test
