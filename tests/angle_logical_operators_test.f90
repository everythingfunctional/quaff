module angle_logical_operators_test
    implicit none
    private
...
contains
    function test_equal_operator() result(tests)
        ...
        individual_tests(1) = it( &
                "is equal for the same angle", &
                DOUBLE_PRECISION_GENERATOR, &
                checkEqualWithSameNumber)
        individual_tests(2) = it( &
                "is not equal for different angles", &
                DOUBLE_PRECISION_GENERATOR, &
                checkEqualWithDifferentNumbers)
        tests = describe("Angle_t with operator(==)", individual_tests)
    ...
    function test_equal_within() result(tests)
        ...
        individual_tests(1) = it( &
                "is equal for the same angle even for tiny tolerance", &
                DOUBLE_PRECISION_GENERATOR, &
        ...
        tests = describe("Angle_t%equal with tolerance", individual_tests)
    ...
    function test_greater_than_or_equal_operator() result(tests)
        ...
        tests = describe("Angle_t operator (>=)", &
                individual_tests)
    ...
    function checkEqualWithSameNumber(number) result(result_)
        ...
        type is (DoublePrecisionInput_t)
            the_angle = number%value_.unit.RADIANS
            result_ = assertThat( &
                    the_angle == the_angle, &
                    the_angle%toString() // " == " // the_angle%toString())
        class default
    ...
    function checkEqualWithDifferentNumbers(number) result(result_)
        ...
        select type (number)
        type is (DoublePrecisionInput_t)
            angle1 = number%value_.unit.RADIANS
            angle2 = (1.0d0 + number%value_).unit.RADIANS
            result_ = assertNot( &
                    angle1 == angle2, &
                    angle1%toString() // " == " // angle2%toString())
        class default
            result_ = fail("Expected to get a DoublePrecisionInput_t")
        end select
    ...
    function checkEqualWithinWithSameNumber(number) result(result_)
        ...
        type is (DoublePrecisionInput_t)
            ...
            result_ = assertThat( &
                    ...
                    "(" // the_angle%toString() // ")%equal(" &
                        // the_angle%toString() // ", within = " &
                        // tolerance%toString() // ")")
        class default
            result_ = fail("Expected to get a DoublePrecisionInput_t")
        end select
    ...
    function checkEqualWithinWithCloseNumbers(number) result(result_)
        ...
        type is (DoublePrecisionInput_t)
            angle1 = number%value_.unit.RADIANS
            angle2 = (number%value_ + 0.05d0).unit.RADIANS
            tolerance = 0.1d0.unit.RADIANS
            result_ = assertThat( &
                    angle1%equal(angle2, within = tolerance), &
                    "(" // angle1%toString() // ")%equal(" &
                        // angle2%toString() // ", within = " &
                        // tolerance%toString() // ")")
        class default
            result_ = fail("Expected to get a DoublePrecisionInput_t")
        end select
    ...
    function checkEqualWithinWithDifferentNumbers(number) result(result_)
        ...
        type is (DoublePrecisionInput_t)
            angle1 = number%value_.unit.RADIANS
            angle2 = (number%value_ + 0.2d0).unit.RADIANS
            tolerance = 0.1d0.unit.RADIANS
            result_ = assertNot( &
                    angle1%equal(angle2, within = tolerance), &
                    "(" // angle1%toString() // ")%equal(" &
    ...
    function checkGreaterThanOrEqualWithGreaterNumbers(number) result(result_)
        ...
        type is (DoublePrecisionInput_t)
            angle1 = number%value_.unit. RADIANS
            angle2 = (number%value_ - 1.0d0).unit.RADIANS
            result_ = assertThat( &
                    angle1 >= angle2, &
                    angle1%toString() // " >= " // angle2%toString())
        class default
    ...
    function checkGreaterThanOrEqualWithSameNumbers(number) result(result_)
        ...
        type is (DoublePrecisionInput_t)
            angle1 = number%value_.unit.RADIANS
            angle2 = number%value_.unit.RADIANS
            result_ = assertThat( &
                    angle1 >= angle2, &
                    angle1%toString() // " >= " // angle2%toString())
        class default
...
end module angle_logical_operators_test
