...
contains
    ...
    function checkRoundTrip(units) result(result_)
        ...
        type is (UnitsInput_t)
            ...
            type is (AmountUnit_t)
                result_ = checkRoundTripIn(the_unit)
            class default
                result_ = fail("Expected to get an AmountUnit_t")
            end select
        class default
    ...
    function checkConversionFactorsInverse(pair) result(result_)
        ...
        type is (UnitsPairInput_t)
            ...
            type is (AmountUnit_t)
                ...
                class default
                    result_ = fail("Expected second in pair to be AmountUnit_t")
                end select
            class default
                result_ = fail("Expected first in pair to be AmountUnit_t")
            end select
        class default
    ...
    function checkToAndFromString(units) result(result_)
        ...
        type is (UnitsInput_t)
            select type (the_unit => units%unit)
            type is (AmountUnit_t)
                original_amount = 3.0d0.unit.the_unit
                new_amount = amountFromString( &
                        original_amount%toStringIn(the_unit), errors)
                result_ = &
                        assertEquals( &
                                original_amount, &
                                new_amount, &
