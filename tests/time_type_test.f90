module time_type_test
    ...
    type, public, extends(Input_t) :: UnitsInput_t
        class(TimeUnit_t), allocatable :: unit
    end type UnitsInput_t

    type, public, extends(Input_t) :: UnitsPairInput_t
        class(TimeUnit_t), allocatable :: first
        class(TimeUnit_t), allocatable :: second
...
contains
    function test_time() result(tests)
        ...
        individual_tests(6) = it( &
                ...
                checkBadNumber)
        tests = describe("Time_t", individual_tests)
    ...
    function checkToAndFromString(units) result(result_)
        ...
        select type (units)
        type is (UnitsInput_t)
            original_time = 3.0d0.unit.units%unit
            new_time = timeFromString( &
                    original_time%toStringIn(units%unit), errors)
            result_ = &
                    assertEquals( &
                            original_time, &
                            new_time, &
