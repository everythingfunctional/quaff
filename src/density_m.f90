module density_m
    ...
    use quaff_conversion_factors_m, only: &
            ...
            POUNDS_PER_CUBIC_FEET_PER_KILOGRAMS_PER_CUBIC_METER
    ...
    type(density_simple_unit_t), parameter :: KILOGRAMS_PER_CUBIC_METER = &
            density_simple_unit_t( &
                    ...
                    symbol = "kg/m^3")
    type(density_simple_unit_t), parameter :: POUNDS_PER_CUBIC_FEET = &
            density_simple_unit_t( &
                    conversion_factor = POUNDS_PER_CUBIC_FEET_PER_KILOGRAMS_PER_CUBIC_METER, &
                    symbol = "lbm/ft^3")
...
contains
    ...
    function parse_density_with_units_s( &
        ...
        type(fallible_density_t) :: all_attempts(size(units))
        integer :: i

        do i = 1, size(units)
            all_attempts(i) = units(i)%parse_as(string)
            if (.not. all_attempts(i)%failed()) then
                fallible_density = all_attempts(i)
                return
            end if
        end do
