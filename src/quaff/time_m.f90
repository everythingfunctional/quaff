module quaff_time_m
    ...
    type :: fallible_time_t
        private
        type(time_t) :: time_ = time_t(0.0d0)
...
contains
    ...
    function parse_time_with_units_s( &
        ...
        do i = 1, size(units)
            fallible_time = units(i)%parse_as(string)
            if (.not. fallible_time%failed()) return
        end do
        fallible_time = fallible_time_t(error_list_t(fatal_t( &
                PARSE_ERROR, &
                module_t(MODULE_NAME), &
                procedure_t("parse_time_with_units_s"), &
                "Unable to parse '" // string // "' as a time_t. Tried with units: " &
