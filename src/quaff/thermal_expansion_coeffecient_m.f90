...
contains
    ...
    function parse_thermal_expansion_coeffecient_with_units_s( &
        ...
        do i = 1, size(units)
            fallible_thermal_expansion_coeffecient = units(i)%parse_as(string)
            if (.not. fallible_thermal_expansion_coeffecient%failed()) return
        end do
        fallible_thermal_expansion_coeffecient = fallible_thermal_expansion_coeffecient_t(error_list_t(fatal_t( &
                ...
                "Unable to parse '" // string // "' as a thermal_expansion_coeffecient_t. Tried with units: " &
