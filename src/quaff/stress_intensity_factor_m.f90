...
contains
    ...
    function parse_stress_intensity_factor_with_units_s( &
        ...
        type(fallible_stress_intensity_factor_t) :: all_attempts(size(units))
        integer :: i

        do i = 1, size(units)
            all_attempts(i) = units(i)%parse_as(string)
            if (.not. all_attempts(i)%failed()) then
                fallible_stress_intensity_factor = all_attempts(i)
                return
            end if
        end do
        fallible_stress_intensity_factor = fallible_stress_intensity_factor_t(error_list_t( &
                all_attempts%errors(), &
                module_t(MODULE_NAME), &
