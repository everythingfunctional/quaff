module quaff_temperature_m
    ...
    type :: fallible_temperature_t
        private
        type(temperature_t) :: temperature_ = temperature_t(0.0d0)
...
contains
    ...
    function parse_temperature_with_units_s( &
        ...
        do i = 1, size(units)
            fallible_temperature = units(i)%parse_as(string)
            if (.not. fallible_temperature%failed()) return
        end do
        fallible_temperature = fallible_temperature_t(error_list_t(fatal_t( &
                PARSE_ERROR, &
                module_t(MODULE_NAME), &
                procedure_t("parse_temperature_with_units_s"), &
                "Unable to parse '" // string // "' as a temperature_t. Tried with units: " &
