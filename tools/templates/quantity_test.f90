module quantity_test
    ...
    implicit none
    private
    public :: test_quantity
contains
    function test_quantity() result(tests)
        type(test_item_t) :: tests

        type(units_examples_t) :: examples

        examples = make_units_examples(PROVIDED_QUANTITY_UNITS)
        tests = describe( &
                "quantity_t", &
                [ it( &
                ...
                , it( &
                        ...
                        check_subtract_zero) &
                , it( &
                        "adding and subtracting the same quantity returns the original quantity", &
                ...
                , describe( &
                        ...
                        , it( &
                                "is false for different quantitys", &
                                DOUBLE_PRECISION_GENERATOR, &
                ...
                , describe( &
                        ...
                        , it( &
                                "is true for different quantitys", &
                                DOUBLE_PRECISION_GENERATOR, &
    ...
    pure function check_conversion_factors_are_inverse( &
        ...
        class(quantity_unit_t), intent(in) :: to
        class(quantity_unit_t), intent(in) :: from
        type(result_t) :: result_
    ...
    contains
        function do_check(input) result(result__)
            ...
            type(quantity_t) :: original_quantity
            type(fallible_quantity_t) :: maybe_quantity
            type(quantity_t) :: new_quantity

            select type (input)
            type is (double_precision_input_t)
                original_quantity = input%input().unit.units
                maybe_quantity = parse_quantity( &
                        original_quantity%to_string_in(units))
                new_quantity = maybe_quantity%quantity()
    ...
    function check_bad_string() result(result_)
        ...
        maybe_quantity = parse_quantity("bad")
        errors = maybe_quantity%errors()
    ...
    function check_bad_number() result(result_)
        ...
        maybe_quantity = parse_quantity("bad SYM")
        errors = maybe_quantity%errors()
        result_ = assert_that(errors.hasType.PARSE_ERROR, errors%to_string())
    end function
    ...
    pure function check_add_zero(input) result(result_)
        ...
        type(quantity_t) :: zero

        select type(input)
        type is (double_precision_input_t)
            quantity = input%input().unit.METERS
    ...
    pure function check_add_subtract(input) result(result_)
        ...
        type(quantity_t) :: quantity1
        type(quantity_t) :: quantity2

        select type(input)
        type is (double_precision_pair_input_t)
            quantity1 = input%first().unit.METERS
            quantity2 = input%second_().unit.METERS
            result_ = assert_equals_within_relative( &
                    quantity1, &
                    (quantity1 + quantity2) - quantity2, &
                    1.0d-8, &
                    "quantity1 = " // quantity1%to_string() &
                    // ", quantity2 = " // quantity2%to_string())
        class default
    ...
    pure function check_multiply_by_one(input) result(result_)
        ...
        type is (double_precision_input_t)
            quantity = input%input().unit.METERS
            result_ = assert_equals(quantity, quantity * 1.0d0)
        class default
            result_ = fail("Expected a double_precision_input_t")
        end select
    end function
    ...
    pure function check_multiply_by_zero(input) result(result_)
        ...
        type is (double_precision_input_t)
            quantity = input%input().unit.METERS
            zero = 0.0d0.unit.METERS
            result_ = assert_equals(zero, quantity * 0.0d0)
        class default
            result_ = fail("Expected a double_precision_input_t")
        end select
    end function
    ...
    pure function check_multiply_divide(input) result(result_)
        ...
        select type (input)
        type is (double_precision_pair_input_t)
            quantity = input%first().unit.METERS
            result_ = assert_equals(quantity, (quantity * input%second_()) / input%second_())
        class default
            result_ = fail("Expected a double_precision_pair_input_t")
        end select
    end function
    ...
    pure function check_equal_with_same_number(input) result(result_)
        ...
        type(quantity_t) :: the_quantity

        select type (input)
        type is (double_precision_input_t)
            the_quantity = input%input().unit.METERS
            result_ = assert_that( &
                    the_quantity == the_quantity, &
                    the_quantity%to_string() // " == " // the_quantity%to_string())
        class default
    ...
    pure function check_equal_with_different_numbers(input) result(result_)
        ...
        select type (input)
        type is (double_precision_input_t)
            quantity1 = input%input().unit.METERS
            quantity2 = (input%input() + 1.0d0).unit.METERS
            result_ = assert_not( &
                    quantity1 == quantity2, &
                    quantity1%to_string() // " == " // quantity2%to_string())
        class default
            result_ = fail("Expected to get a double_precision_input_t")
        end select
    end function
    ...
    pure function check_equal_within_with_same_number(input) result(result_)
        ...
        type is (double_precision_input_t)
            ...
            result_ = assert_that( &
                    the_quantity%equal(the_quantity, within = tolerance), &
                    "(" // the_quantity%to_string() // ")%equal(" &
                        // the_quantity%to_string() // ", within = " &
                        // tolerance%to_string() // ")")
        class default
            result_ = fail("Expected to get a double_precision_input_t")
        end select
    ...
    pure function check_equal_within_with_close_numbers(input) result(result_)
        ...
        type is (double_precision_input_t)
            quantity1 = input%input().unit.METERS
            quantity2 = (input%input() + 0.05d0).unit.METERS
            tolerance = 0.1d0.unit.METERS
            result_ = assert_that( &
                    quantity1%equal(quantity2, within = tolerance), &
                    "(" // quantity1%to_string() // ")%equal(" &
                        // quantity2%to_string() // ", within = " &
                        // tolerance%to_string() // ")")
        class default
            result_ = fail("Expected to get a double_precision_input_t")
        end select
    ...
    pure function check_equal_within_with_different_numbers(input) result(result_)
        ...
        type is (double_precision_input_t)
            quantity1 = input%input().unit.METERS
            quantity2 = (input%input() + 0.2d0).unit.METERS
            tolerance = 0.1d0.unit.METERS
            result_ = assert_not( &
                    quantity1%equal(quantity2, within = tolerance), &
                    "(" // quantity1%to_string() // ")%equal(" &
    ...
    pure function check_greater_than_or_equal_with_same_numbers(input) result(result_)
        ...
        type is (double_precision_input_t)
            quantity1 = input%input().unit.METERS
            quantity2 = input%input().unit.METERS
            result_ = assert_that( &
                    quantity1 >= quantity2, &
                    quantity1%to_string() // " >= " // quantity2%to_string())
        class default
