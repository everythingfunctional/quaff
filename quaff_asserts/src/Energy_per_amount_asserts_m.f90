module Energy_per_amount_asserts_m
    use iso_varying_string, only: VARYING_STRING, operator(//), var_str
    use quaff, only: EnergyPerAmount_t
...
contains
    pure function assertEqualsBasic(expected, actual) result(result__)
        class(EnergyPerAmount_t), intent(in) :: expected
        class(EnergyPerAmount_t), intent(in) :: actual
    ...
    pure function assertEqualsWithinAbsoluteBasic( &
        ...
        class(EnergyPerAmount_t), intent(in) :: expected
        class(EnergyPerAmount_t), intent(in) :: actual
        class(EnergyPerAmount_t), intent(in) :: tolerance
        type(Result_t) :: result__
...
end module Energy_per_amount_asserts_m
