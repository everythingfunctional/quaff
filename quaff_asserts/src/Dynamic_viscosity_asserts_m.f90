module Dynamic_viscosity_asserts_m
    use iso_varying_string, only: VARYING_STRING, operator(//), var_str
    use quaff, only: DynamicViscosity_t
...
contains
    pure function assertEqualsBasic(expected, actual) result(result__)
        class(DynamicViscosity_t), intent(in) :: expected
        class(DynamicViscosity_t), intent(in) :: actual
    ...
    pure function assertEqualsWithinAbsoluteBasic( &
        ...
        class(DynamicViscosity_t), intent(in) :: expected
        class(DynamicViscosity_t), intent(in) :: actual
        class(DynamicViscosity_t), intent(in) :: tolerance
        type(Result_t) :: result__
...
end module Dynamic_viscosity_asserts_m
