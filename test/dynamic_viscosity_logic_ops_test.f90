module dynamic_viscosity_logic_ops_test
...
contains
    function test_equal_operator() result(tests)
        ...
        individual_tests(1) = it( &
                "is equal for the same dynamic_viscosity", &
                DOUBLE_PRECISION_GENERATOR, &
                checkEqualWithSameNumber)
        individual_tests(2) = it( &
                "is not equal for different dynamic_viscositys", &
                DOUBLE_PRECISION_GENERATOR, &
                checkEqualWithDifferentNumbers)
        tests = describe("DynamicViscosity_t with operator(==)", individual_tests)
    ...
    function test_equal_within() result(tests)
        ...
        individual_tests(1) = it( &
                "is equal for the same dynamic_viscosity even for tiny tolerance", &
                DOUBLE_PRECISION_GENERATOR, &
        ...
        tests = describe("DynamicViscosity_t%equal with tolerance", individual_tests)
    ...
    function test_greater_than_or_equal_operator() result(tests)
        ...
        tests = describe("DynamicViscosity_t operator (>=)", &
                individual_tests)
    ...
    pure function checkEqualWithSameNumber(number) result(result_)
        ...
        type(DynamicViscosity_t) :: the_dynamic_viscosity

        select type (number)
        type is (DoublePrecisionInput_t)
            the_dynamic_viscosity = number%value_.unit.PASCAL_SECONDS
            result_ = assertThat( &
                    the_dynamic_viscosity == the_dynamic_viscosity, &
                    the_dynamic_viscosity%toString() // " == " // the_dynamic_viscosity%toString())
        class default
    ...
    pure function checkEqualWithDifferentNumbers(number) result(result_)
        ...
        select type (number)
        type is (DoublePrecisionInput_t)
            dynamic_viscosity1 = number%value_.unit.PASCAL_SECONDS
            dynamic_viscosity2 = (1.0d0 + number%value_).unit.PASCAL_SECONDS
            result_ = assertNot( &
                    dynamic_viscosity1 == dynamic_viscosity2, &
                    dynamic_viscosity1%toString() // " == " // dynamic_viscosity2%toString())
        class default
            result_ = fail("Expected to get a DoublePrecisionInput_t")
        end select
    ...
    pure function checkEqualWithinWithSameNumber(number) result(result_)
        ...
        type(DynamicViscosity_t) :: the_dynamic_viscosity
        type(DynamicViscosity_t) :: tolerance
        ...
        type is (DoublePrecisionInput_t)
            ...
            result_ = assertThat( &
                    ...
                    "(" // the_dynamic_viscosity%toString() // ")%equal(" &
                        // the_dynamic_viscosity%toString() // ", within = " &
                        // tolerance%toString() // ")")
        class default
            result_ = fail("Expected to get a DoublePrecisionInput_t")
        end select
    ...
    pure function checkEqualWithinWithCloseNumbers(number) result(result_)
        ...
        type is (DoublePrecisionInput_t)
            dynamic_viscosity1 = number%value_.unit.PASCAL_SECONDS
            dynamic_viscosity2 = (number%value_ + 0.05d0).unit.PASCAL_SECONDS
            tolerance = 0.1d0.unit.PASCAL_SECONDS
            result_ = assertThat( &
                    dynamic_viscosity1%equal(dynamic_viscosity2, within = tolerance), &
                    "(" // dynamic_viscosity1%toString() // ")%equal(" &
                        // dynamic_viscosity2%toString() // ", within = " &
                        // tolerance%toString() // ")")
        class default
            result_ = fail("Expected to get a DoublePrecisionInput_t")
        end select
    ...
    pure function checkEqualWithinWithDifferentNumbers(number) result(result_)
        ...
        type is (DoublePrecisionInput_t)
            dynamic_viscosity1 = number%value_.unit.PASCAL_SECONDS
            dynamic_viscosity2 = (number%value_ + 0.2d0).unit.PASCAL_SECONDS
            tolerance = 0.1d0.unit.PASCAL_SECONDS
            result_ = assertNot( &
                    dynamic_viscosity1%equal(dynamic_viscosity2, within = tolerance), &
                    "(" // dynamic_viscosity1%toString() // ")%equal(" &
    ...
    pure function checkGreaterThanOrEqualWithGreaterNumbers(number) result(result_)
        ...
        type is (DoublePrecisionInput_t)
            dynamic_viscosity1 = number%value_.unit. PASCAL_SECONDS
            dynamic_viscosity2 = (number%value_ - 1.0d0).unit.PASCAL_SECONDS
            result_ = assertThat( &
                    dynamic_viscosity1 >= dynamic_viscosity2, &
                    dynamic_viscosity1%toString() // " >= " // dynamic_viscosity2%toString())
        class default
    ...
    pure function checkGreaterThanOrEqualWithSameNumbers(number) result(result_)
        ...
        type is (DoublePrecisionInput_t)
            dynamic_viscosity1 = number%value_.unit.PASCAL_SECONDS
            dynamic_viscosity2 = number%value_.unit.PASCAL_SECONDS
            result_ = assertThat( &
                    dynamic_viscosity1 >= dynamic_viscosity2, &
                    dynamic_viscosity1%toString() // " >= " // dynamic_viscosity2%toString())
        class default
...
end module dynamic_viscosity_logic_ops_test
