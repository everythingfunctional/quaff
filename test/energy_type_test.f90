module energy_type_test
    ...
    type, public, extends(Input_t) :: UnitsInput_t
        class(EnergyUnit_t), allocatable :: unit
    end type UnitsInput_t

    type, public, extends(Input_t) :: UnitsPairInput_t
        class(EnergyUnit_t), allocatable :: first
        class(EnergyUnit_t), allocatable :: second
...
contains
    function test_energy() result(tests)
        ...
        examples = makeUnitsExamples(PROVIDED_ENERGY_UNITS)
        ...
        tests = describe("Energy_t", individual_tests)
    end function test_energy
    ...
    pure function checkBadString() result(result_)
        ...
        call fromString("bad", errors, energy)
    ...
    pure function checkBadNumber() result(result_)
        ...
        call fromString("bad J", errors, energy)
        result_ = assertThat(errors.hasType.PARSE_ERROR, errors%toString())
    ...
    pure function checkConversionFactorsAreInverse( &
        ...
        class(EnergyUnit_t), intent(in) :: to
        class(EnergyUnit_t), intent(in) :: from
        type(Result_t) :: result_
    ...
    contains
        pure function doCheck(input) result(result__)
            ...
            select type (input)
            type is (DoublePrecisionInput_t)
                original_energy = input%value_.unit.units
                call fromString( &
                        original_energy%toStringIn(units), &
                        errors, &
                        new_energy)
                result__ = &
                        assertEquals( &
