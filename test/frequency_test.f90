module frequency_test
    ...
    implicit none
    private
    public :: test_frequency
contains
    function test_frequency() result(tests)
        type(test_item_t) :: tests

        type(units_examples_t) :: examples

        examples = make_units_examples(PROVIDED_FREQUENCY_UNITS)
        tests = describe( &
                "frequency_t", &
                [ it( &
                ...
                , it( &
                        "adding zero returns the original frequency", &
                        DOUBLE_PRECISION_GENERATOR, &
                        check_add_zero) &
                , it( &
                        "subtracting zero returns the original frequency", &
                        DOUBLE_PRECISION_GENERATOR, &
                        check_subtract_zero) &
                , it( &
                        "adding and subtracting the same frequency returns the original frequency", &
                        DOUBLE_PRECISION_PAIR_GENERATOR, &
                        check_add_subtract) &
                , it( &
                        "multiplying by one returns the original frequency", &
                        DOUBLE_PRECISION_GENERATOR, &
                        check_multiply_by_one) &
                , it( &
                        "multiplying by zero returns zero frequency", &
                        DOUBLE_PRECISION_GENERATOR, &
                        check_multiply_by_zero) &
                , it( &
                        "dividing by one returns the original frequency", &
                        DOUBLE_PRECISION_GENERATOR, &
                        check_divide_by_one) &
                , it( &
                ...
                , it( &
                        "multiplying and dividing by the same number returns the original frequency", &
                        NON_ZERO_DOUBLE_PRECISION_PAIR_GENERATOR, &
                        check_multiply_divide) &
                , describe( &
                        "operator(==)", &
                        [ it( &
                                "is true for the same frequency", &
                                DOUBLE_PRECISION_GENERATOR, &
                                check_equal_with_same_number) &
                        , it( &
                                "is false for different frequencys", &
                                DOUBLE_PRECISION_GENERATOR, &
                ...
                , describe( &
                        "operator(/=)", &
                        [ it( &
                                "is false for the same frequency", &
                                DOUBLE_PRECISION_GENERATOR, &
                                check_not_equal_with_same_number) &
                        , it( &
                                "is true for different frequencys", &
                                DOUBLE_PRECISION_GENERATOR, &
                                check_not_equal_with_different_numbers) &
                        ]) &
                , describe( &
                        "%equal(frequency, within)", &
                        [ it( &
                                "is true for the same frequency even for tiny tolerance", &
                                DOUBLE_PRECISION_GENERATOR, &
    ...
    pure function check_conversion_factors_are_inverse( &
        ...
        class(frequency_unit_t), intent(in) :: to
        class(frequency_unit_t), intent(in) :: from
        type(result_t) :: result_
    ...
    contains
        function do_check(input) result(result__)
            ...
            type(frequency_t) :: original_frequency
            type(fallible_frequency_t) :: maybe_frequency
            type(frequency_t) :: new_frequency

            select type (input)
            type is (double_precision_input_t)
                original_frequency = input%input().unit.units
                maybe_frequency = parse_frequency( &
                        original_frequency%to_string_in(units))
                new_frequency = maybe_frequency%frequency()
    ...
    function check_bad_string() result(result_)
        ...
        maybe_frequency = parse_frequency("bad")
        errors = maybe_frequency%errors()
    ...
    function check_bad_number() result(result_)
        ...
        maybe_frequency = parse_frequency("bad Hz")
        errors = maybe_frequency%errors()
        result_ = assert_that(errors.hasType.PARSE_ERROR, errors%to_string())
    end function
    ...
    pure function check_add_zero(input) result(result_)
        ...
        type(frequency_t) :: zero

        select type(input)
        type is (double_precision_input_t)
            frequency = input%input().unit.HERTZ
            zero = 0.0d0.unit.HERTZ
            result_ = assert_equals(frequency, frequency + zero)
        class default
            result_ = fail("Expected a double_precision_input_t")
        end select
    end function
    ...
    pure function check_add_subtract(input) result(result_)
        ...
        type(frequency_t) :: frequency1
        type(frequency_t) :: frequency2

        select type(input)
        type is (double_precision_pair_input_t)
            frequency1 = input%first().unit.HERTZ
            frequency2 = input%second_().unit.HERTZ
            result_ = assert_equals_within_relative( &
                    frequency1, &
                    (frequency1 + frequency2) - frequency2, &
                    1.0d-8, &
    ...
    pure function check_multiply_by_one(input) result(result_)
        ...
        type is (double_precision_input_t)
            frequency = input%input().unit.HERTZ
            result_ = assert_equals(frequency, frequency * 1.0d0)
        class default
            result_ = fail("Expected a double_precision_input_t")
        end select
    end function
    ...
    pure function check_multiply_by_zero(input) result(result_)
        ...
        type is (double_precision_input_t)
            frequency = input%input().unit.HERTZ
            zero = 0.0d0.unit.HERTZ
            result_ = assert_equals(zero, frequency * 0.0d0)
        class default
            result_ = fail("Expected a double_precision_input_t")
        end select
    end function
    ...
    pure function check_divide_by_self(input) result(result_)
        ...
        type is (double_precision_input_t)
            frequency = input%input().unit.HERTZ
            result_ = assert_equals(1.0d0, frequency / frequency)
        class default
            result_ = fail("Expected a double_precision_input_t")
        end select
    end function
    ...
    pure function check_multiply_divide(input) result(result_)
        ...
        select type (input)
        type is (double_precision_pair_input_t)
            frequency = input%first().unit.HERTZ
            result_ = assert_equals(frequency, (frequency * input%second_()) / input%second_())
        class default
            result_ = fail("Expected a double_precision_pair_input_t")
        end select
    end function
    ...
    pure function check_equal_with_same_number(input) result(result_)
        ...
        type(frequency_t) :: the_frequency

        select type (input)
        type is (double_precision_input_t)
            the_frequency = input%input().unit.HERTZ
            result_ = assert_that( &
                    the_frequency == the_frequency, &
                    the_frequency%to_string() // " == " // the_frequency%to_string())
        class default
    ...
    pure function check_equal_with_different_numbers(input) result(result_)
        ...
        select type (input)
        type is (double_precision_input_t)
            frequency1 = input%input().unit.HERTZ
            frequency2 = (input%input() + 1.0d0).unit.HERTZ
            result_ = assert_not( &
                    frequency1 == frequency2, &
                    frequency1%to_string() // " == " // frequency2%to_string())
        class default
            result_ = fail("Expected to get a double_precision_input_t")
        end select
    end function
    ...
    pure function check_equal_within_with_same_number(input) result(result_)
        ...
        type(frequency_t) :: the_frequency
        type(frequency_t) :: tolerance

        select type (input)
        type is (double_precision_input_t)
            the_frequency = input%input().unit.HERTZ
            tolerance = tiny(1.0d0).unit.HERTZ
            result_ = assert_that( &
                    the_frequency%equal(the_frequency, within = tolerance), &
                    "(" // the_frequency%to_string() // ")%equal(" &
                        // the_frequency%to_string() // ", within = " &
                        // tolerance%to_string() // ")")
        class default
            result_ = fail("Expected to get a double_precision_input_t")
        end select
    ...
    pure function check_equal_within_with_close_numbers(input) result(result_)
        ...
        type is (double_precision_input_t)
            frequency1 = input%input().unit.HERTZ
            frequency2 = (input%input() + 0.05d0).unit.HERTZ
            tolerance = 0.1d0.unit.HERTZ
            result_ = assert_that( &
                    frequency1%equal(frequency2, within = tolerance), &
                    "(" // frequency1%to_string() // ")%equal(" &
                        // frequency2%to_string() // ", within = " &
                        // tolerance%to_string() // ")")
        class default
            result_ = fail("Expected to get a double_precision_input_t")
        end select
    ...
    pure function check_equal_within_with_different_numbers(input) result(result_)
        ...
        type is (double_precision_input_t)
            frequency1 = input%input().unit.HERTZ
            frequency2 = (input%input() + 0.2d0).unit.HERTZ
            tolerance = 0.1d0.unit.HERTZ
            result_ = assert_not( &
                    frequency1%equal(frequency2, within = tolerance), &
                    "(" // frequency1%to_string() // ")%equal(" &
    ...
    pure function check_greater_than_or_equal_with_same_numbers(input) result(result_)
        ...
        type is (double_precision_input_t)
            frequency1 = input%input().unit.HERTZ
            frequency2 = input%input().unit.HERTZ
            result_ = assert_that( &
                    frequency1 >= frequency2, &
                    frequency1%to_string() // " >= " // frequency2%to_string())
        class default
