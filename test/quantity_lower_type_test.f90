module quantity_lower_type_test
    ...
    use quaff, only: &
            ...
            fromString, &
            sum, &
            PROVIDED_QUANTITY_CAPITAL_UNITS, &
            UNITS_CAPITAL
...
contains
    function test_quantity_lower() result(tests)
        ...
        examples = makeUnitsExamples(PROVIDED_QUANTITY_CAPITAL_UNITS)
    ...
    pure function checkSum() result(result_)
        ...
        result_ = assertEquals( &
                sum(numbers).unit.UNITS_CAPITAL, &
                sum(numbers.unit.UNITS_CAPITAL))
    ...
    pure function checkConversionFactorsAreInverse( &
        ...
        class(QuantityCamelUnit_t), intent(in) :: to
        class(QuantityCamelUnit_t), intent(in) :: from
        type(Result_t) :: result_
