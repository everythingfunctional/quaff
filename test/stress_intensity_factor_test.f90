module stress_intensity_factor_test
    ...
    implicit none
    private
    public :: test_stress_intensity_factor
contains
    function test_stress_intensity_factor() result(tests)
        type(test_item_t) :: tests

        type(units_examples_t) :: examples

        examples = make_units_examples(PROVIDED_STRESS_INTENSITY_FACTOR_UNITS)
        tests = describe( &
                "stress_intensity_factor_t", &
                [ it( &
                ...
                , it( &
                        "adding zero returns the original stress_intensity_factor", &
                        DOUBLE_PRECISION_GENERATOR, &
                        check_add_zero) &
                , it( &
                        "subtracting zero returns the original stress_intensity_factor", &
                        DOUBLE_PRECISION_GENERATOR, &
                        check_subtract_zero) &
                , it( &
                        "adding and subtracting the same stress_intensity_factor returns the original stress_intensity_factor", &
                        DOUBLE_PRECISION_PAIR_GENERATOR, &
                        check_add_subtract) &
                , it( &
                        "multiplying by one returns the original stress_intensity_factor", &
                        DOUBLE_PRECISION_GENERATOR, &
                        check_multiply_by_one) &
                , it( &
                        "multiplying by zero returns zero stress_intensity_factor", &
                        DOUBLE_PRECISION_GENERATOR, &
                        check_multiply_by_zero) &
                , it( &
                        "dividing by one returns the original stress_intensity_factor", &
                        DOUBLE_PRECISION_GENERATOR, &
                        check_divide_by_one) &
                , it( &
                ...
                , it( &
                        "multiplying and dividing by the same number returns the original stress_intensity_factor", &
                        NON_ZERO_DOUBLE_PRECISION_PAIR_GENERATOR, &
                        check_multiply_divide) &
                , describe( &
                        "operator(==)", &
                        [ it( &
                                "is true for the same stress_intensity_factor", &
                                DOUBLE_PRECISION_GENERATOR, &
                                check_equal_with_same_number) &
                        , it( &
                                "is false for different stress_intensity_factors", &
                                DOUBLE_PRECISION_GENERATOR, &
                ...
                , describe( &
                        "operator(/=)", &
                        [ it( &
                                "is false for the same stress_intensity_factor", &
                                DOUBLE_PRECISION_GENERATOR, &
                                check_not_equal_with_same_number) &
                        , it( &
                                "is true for different stress_intensity_factors", &
                                DOUBLE_PRECISION_GENERATOR, &
                                check_not_equal_with_different_numbers) &
                        ]) &
                , describe( &
                        "%equal(stress_intensity_factor, within)", &
                        [ it( &
                                "is true for the same stress_intensity_factor even for tiny tolerance", &
                                DOUBLE_PRECISION_GENERATOR, &
    ...
    contains
        pure function check_round_trip_(input) result(result__)
            ...
            type(stress_intensity_factor_t) :: intermediate
    ...
    pure function check_conversion_factors_are_inverse( &
        ...
        class(stress_intensity_factor_unit_t), intent(in) :: to
        class(stress_intensity_factor_unit_t), intent(in) :: from
        type(result_t) :: result_
    ...
    contains
        function do_check(input) result(result__)
            ...
            type(stress_intensity_factor_t) :: original_stress_intensity_factor
            ...
            select type (input)
            type is (double_precision_input_t)
                original_stress_intensity_factor = input%input().unit.units
                maybe_stress_intensity_factor = parse_stress_intensity_factor( &
                        original_stress_intensity_factor%to_string_in(units))
                new_stress_intensity_factor = maybe_stress_intensity_factor%stress_intensity_factor()
                errors = maybe_stress_intensity_factor%errors()
                result__ = &
                        assert_equals( &
                                original_stress_intensity_factor, &
                                new_stress_intensity_factor) &
    ...
    function check_bad_string() result(result_)
        ...
        maybe_stress_intensity_factor = parse_stress_intensity_factor("bad")
        errors = maybe_stress_intensity_factor%errors()
    ...
    pure function check_add_zero(input) result(result_)
        ...
        type(stress_intensity_factor_t) :: zero
    ...
    pure function check_add_subtract(input) result(result_)
        ...
        type(stress_intensity_factor_t) :: stress_intensity_factor1
        type(stress_intensity_factor_t) :: stress_intensity_factor2
        ...
        type is (double_precision_pair_input_t)
            ...
            result_ = assert_equals_within_relative( &
                    ...
                    1.0d-8, &
                    "stress_intensity_factor1 = " // stress_intensity_factor1%to_string() &
                    // ", stress_intensity_factor2 = " // stress_intensity_factor2%to_string())
        class default
    ...
    pure function check_equal_with_same_number(input) result(result_)
        ...
        type(stress_intensity_factor_t) :: the_stress_intensity_factor
    ...
    pure function check_equal_within_with_same_number(input) result(result_)
        ...
        type(stress_intensity_factor_t) :: the_stress_intensity_factor
        type(stress_intensity_factor_t) :: tolerance
        ...
        type is (double_precision_input_t)
            ...
            result_ = assert_that( &
                    ...
                    "(" // the_stress_intensity_factor%to_string() // ")%equal(" &
                        // the_stress_intensity_factor%to_string() // ", within = " &
                        // tolerance%to_string() // ")")
        class default
            result_ = fail("Expected to get a double_precision_input_t")
        end select
        ...
        type is (double_precision_input_t)
            ...
            result_ = assert_that( &
                    ...
                    "(" // stress_intensity_factor1%to_string() // ")%equal(" &
                        // stress_intensity_factor2%to_string() // ", within = " &
                        // tolerance%to_string() // ")")
        class default
            result_ = fail("Expected to get a double_precision_input_t")
        end select
