module enthalpy_test
    ...
    implicit none
    private
    public :: test_enthalpy
contains
    function test_enthalpy() result(tests)
        type(test_item_t) :: tests

        type(units_examples_t) :: examples

        examples = make_units_examples(PROVIDED_ENTHALPY_UNITS)
        tests = describe( &
                "enthalpy_t", &
                [ it( &
                ...
                , it( &
                        ...
                        check_add_zero) &
                , it( &
                        "subtracting zero returns the original enthalpy", &
                        DOUBLE_PRECISION_GENERATOR, &
                        check_subtract_zero) &
                , it( &
                        "adding and subtracting the same enthalpy returns the original enthalpy", &
                        DOUBLE_PRECISION_PAIR_GENERATOR, &
                        check_add_subtract) &
                , it( &
                        "multiplying by one returns the original enthalpy", &
                        DOUBLE_PRECISION_GENERATOR, &
                        check_multiply_by_one) &
                , it( &
                        "multiplying by zero returns zero enthalpy", &
                        DOUBLE_PRECISION_GENERATOR, &
                        check_multiply_by_zero) &
                , it( &
                        "dividing by one returns the original enthalpy", &
                        DOUBLE_PRECISION_GENERATOR, &
                        check_divide_by_one) &
                , it( &
                ...
                , it( &
                        "multiplying and dividing by the same number returns the original enthalpy", &
                        NON_ZERO_DOUBLE_PRECISION_PAIR_GENERATOR, &
                        check_multiply_divide) &
                , describe( &
                        "operator(==)", &
                        [ it( &
                                "is true for the same enthalpy", &
                                DOUBLE_PRECISION_GENERATOR, &
                                check_equal_with_same_number) &
                        , it( &
                                "is false for different enthalpys", &
                                DOUBLE_PRECISION_GENERATOR, &
                ...
                , describe( &
                        "operator(/=)", &
                        [ it( &
                                "is false for the same enthalpy", &
                                DOUBLE_PRECISION_GENERATOR, &
                                check_not_equal_with_same_number) &
                        , it( &
                                "is true for different enthalpys", &
                                DOUBLE_PRECISION_GENERATOR, &
                                check_not_equal_with_different_numbers) &
                        ]) &
                , describe( &
                        "%equal(enthalpy, within)", &
                        [ it( &
                                "is true for the same enthalpy even for tiny tolerance", &
                                DOUBLE_PRECISION_GENERATOR, &
    ...
    pure function check_conversion_factors_are_inverse( &
        ...
        class(enthalpy_unit_t), intent(in) :: to
        class(enthalpy_unit_t), intent(in) :: from
        type(result_t) :: result_
    ...
    contains
        function do_check(input) result(result__)
            ...
            type(enthalpy_t) :: original_enthalpy
            type(fallible_enthalpy_t) :: maybe_enthalpy
            type(enthalpy_t) :: new_enthalpy

            select type (input)
            type is (double_precision_input_t)
                original_enthalpy = input%input().unit.units
                maybe_enthalpy = parse_enthalpy( &
                        original_enthalpy%to_string_in(units))
                new_enthalpy = maybe_enthalpy%enthalpy()
    ...
    function check_bad_string() result(result_)
        ...
        maybe_enthalpy = parse_enthalpy("bad")
        errors = maybe_enthalpy%errors()
    ...
    function check_bad_number() result(result_)
        ...
        maybe_enthalpy = parse_enthalpy("bad J/kg")
        errors = maybe_enthalpy%errors()
        result_ = assert_that(errors.hasType.PARSE_ERROR, errors%to_string())
    end function
    ...
    pure function check_negation(input) result(result_)
        ...
        type is (double_precision_input_t)
            result_ = assert_equals( &
                    (-input%input()).unit.JOULES_PER_KILOGRAM, &
                    -(input%input().unit.JOULES_PER_KILOGRAM))
        class default
    ...
    pure function check_add_zero(input) result(result_)
        ...
        type(enthalpy_t) :: zero

        select type(input)
        type is (double_precision_input_t)
            enthalpy = input%input().unit.JOULES_PER_KILOGRAM
            zero = 0.0d0.unit.JOULES_PER_KILOGRAM
            result_ = assert_equals(enthalpy, enthalpy + zero)
        class default
            result_ = fail("Expected a double_precision_input_t")
        end select
    end function
    ...
    pure function check_add_subtract(input) result(result_)
        ...
        type is (double_precision_pair_input_t)
            enthalpy1 = input%first().unit.JOULES_PER_KILOGRAM
            enthalpy2 = input%second_().unit.JOULES_PER_KILOGRAM
            result_ = assert_equals_within_relative( &
                    enthalpy1, &
                    (enthalpy1 + enthalpy2) - enthalpy2, &
                    1.0d-8, &
                    "enthalpy1 = " // enthalpy1%to_string() &
                    // ", enthalpy2 = " // enthalpy2%to_string())
        class default
    ...
    pure function check_multiply_by_one(input) result(result_)
        ...
        type is (double_precision_input_t)
            enthalpy = input%input().unit.JOULES_PER_KILOGRAM
            result_ = assert_equals(enthalpy, enthalpy * 1.0d0)
        class default
            result_ = fail("Expected a double_precision_input_t")
        end select
    end function
    ...
    pure function check_multiply_by_zero(input) result(result_)
        ...
        type is (double_precision_input_t)
            enthalpy = input%input().unit.JOULES_PER_KILOGRAM
            zero = 0.0d0.unit.JOULES_PER_KILOGRAM
            result_ = assert_equals(zero, enthalpy * 0.0d0)
        class default
            result_ = fail("Expected a double_precision_input_t")
        end select
    end function
    ...
    pure function check_divide_by_self(input) result(result_)
        ...
        type is (double_precision_input_t)
            enthalpy = input%input().unit.JOULES_PER_KILOGRAM
            result_ = assert_equals(1.0d0, enthalpy / enthalpy)
        class default
            result_ = fail("Expected a double_precision_input_t")
        end select
    end function
    ...
    pure function check_multiply_divide(input) result(result_)
        ...
        select type (input)
        type is (double_precision_pair_input_t)
            enthalpy = input%first().unit.JOULES_PER_KILOGRAM
            result_ = assert_equals(enthalpy, (enthalpy * input%second_()) / input%second_())
        class default
            result_ = fail("Expected a double_precision_pair_input_t")
        end select
    end function
    ...
    pure function check_equal_with_same_number(input) result(result_)
        ...
        select type (input)
        type is (double_precision_input_t)
            the_enthalpy = input%input().unit.JOULES_PER_KILOGRAM
            result_ = assert_that( &
                    the_enthalpy == the_enthalpy, &
                    the_enthalpy%to_string() // " == " // the_enthalpy%to_string())
        class default
    ...
    pure function check_equal_with_different_numbers(input) result(result_)
        ...
        select type (input)
        type is (double_precision_input_t)
            enthalpy1 = input%input().unit.JOULES_PER_KILOGRAM
            enthalpy2 = (input%input() + 1.0d0).unit.JOULES_PER_KILOGRAM
            result_ = assert_not( &
                    enthalpy1 == enthalpy2, &
                    enthalpy1%to_string() // " == " // enthalpy2%to_string())
        class default
            result_ = fail("Expected to get a double_precision_input_t")
        end select
    end function
    ...
    pure function check_equal_within_with_same_number(input) result(result_)
        ...
        type is (double_precision_input_t)
            ...
            result_ = assert_that( &
                    ...
                    "(" // the_enthalpy%to_string() // ")%equal(" &
                        // the_enthalpy%to_string() // ", within = " &
                        // tolerance%to_string() // ")")
        class default
            result_ = fail("Expected to get a double_precision_input_t")
        end select
    ...
    pure function check_equal_within_with_close_numbers(input) result(result_)
        ...
        type is (double_precision_input_t)
            enthalpy1 = input%input().unit.JOULES_PER_KILOGRAM
            enthalpy2 = (input%input() + 0.05d0).unit.JOULES_PER_KILOGRAM
            tolerance = 0.1d0.unit.JOULES_PER_KILOGRAM
            result_ = assert_that( &
                    enthalpy1%equal(enthalpy2, within = tolerance), &
                    "(" // enthalpy1%to_string() // ")%equal(" &
                        // enthalpy2%to_string() // ", within = " &
                        // tolerance%to_string() // ")")
        class default
            result_ = fail("Expected to get a double_precision_input_t")
        end select
    ...
    pure function check_equal_within_with_different_numbers(input) result(result_)
        ...
        type is (double_precision_input_t)
            enthalpy1 = input%input().unit.JOULES_PER_KILOGRAM
            enthalpy2 = (input%input() + 0.2d0).unit.JOULES_PER_KILOGRAM
            tolerance = 0.1d0.unit.JOULES_PER_KILOGRAM
            result_ = assert_not( &
                    enthalpy1%equal(enthalpy2, within = tolerance), &
                    "(" // enthalpy1%to_string() // ")%equal(" &
    ...
    pure function check_greater_than_or_equal_with_same_numbers(input) result(result_)
        ...
        type is (double_precision_input_t)
            enthalpy1 = input%input().unit.JOULES_PER_KILOGRAM
            enthalpy2 = input%input().unit.JOULES_PER_KILOGRAM
            result_ = assert_that( &
                    enthalpy1 >= enthalpy2, &
                    enthalpy1%to_string() // " >= " // enthalpy2%to_string())
        class default
