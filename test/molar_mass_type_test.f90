module molar_mass_type_test
    ...
    type, public, extends(Input_t) :: UnitsInput_t
        class(MolarMassUnit_t), allocatable :: unit
    end type UnitsInput_t

    type, public, extends(Input_t) :: UnitsPairInput_t
        class(MolarMassUnit_t), allocatable :: first
        class(MolarMassUnit_t), allocatable :: second
...
contains
    function test_molar_mass() result(tests)
        ...
        examples = makeUnitsExamples(PROVIDED_MOLAR_MASS_UNITS)
        ...
        tests = describe("MolarMass_t", individual_tests)
    end function test_molar_mass
    ...
    pure function checkBadString() result(result_)
        ...
        call fromString("bad", errors, molar_mass)
    ...
    pure function checkBadUnit() result(result_)
        ...
        call fromString( &
                "1.0 bad", [KILOGRAMS_PER_MOL], errors, molar_mass)
        result_ = assertThat(errors.hasType.PARSE_ERROR, errors%toString())
    ...
    pure function checkBadNumber() result(result_)
        ...
        call fromString("bad kg/mol", errors, molar_mass)
        result_ = assertThat(errors.hasType.PARSE_ERROR, errors%toString())
    ...
    pure function checkConversionFactorsAreInverse( &
        ...
        class(MolarMassUnit_t), intent(in) :: to
        class(MolarMassUnit_t), intent(in) :: from
        type(Result_t) :: result_
    ...
    contains
        pure function doCheck(input) result(result__)
            ...
            type(ErrorList_t) :: errors
            type(MolarMass_t) :: original_molar_mass
            type(MolarMass_t) :: new_molar_mass

            select type (input)
            type is (DoublePrecisionInput_t)
                original_molar_mass = input%value_.unit.units
                call fromString( &
                        original_molar_mass%toStringIn(units), &
                        errors, &
                        new_molar_mass)
                result__ = &
                        assertEquals( &
...
end module molar_mass_type_test
