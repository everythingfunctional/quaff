module molar_enthalpy_test
    ...
    use molar_enthalpy_utilities_m, only: &
    ...
    implicit none
    private
    public :: test_molar_enthalpy
contains
    function test_molar_enthalpy() result(tests)
        ...
        tests = describe( &
                ...
                , it( &
                        "adding zero returns the original molar_enthalpy", &
                        DOUBLE_PRECISION_GENERATOR, &
                        check_add_zero) &
                , it( &
                        "subtracting zero returns the original molar_enthalpy", &
                        DOUBLE_PRECISION_GENERATOR, &
                        check_subtract_zero) &
                , it( &
                        "adding and subtracting the same molar_enthalpy returns the original molar_enthalpy", &
                        DOUBLE_PRECISION_PAIR_GENERATOR, &
                        check_add_subtract) &
                , it( &
                        "multiplying by one returns the original molar_enthalpy", &
                        DOUBLE_PRECISION_GENERATOR, &
                        check_multiply_by_one) &
                , it( &
                        "multiplying by zero returns zero molar_enthalpy", &
                        DOUBLE_PRECISION_GENERATOR, &
                        check_multiply_by_zero) &
                , it( &
                        "dividing by one returns the original molar_enthalpy", &
                        DOUBLE_PRECISION_GENERATOR, &
                        check_divide_by_one) &
                , it( &
                ...
                , it( &
                        "multiplying and dividing by the same number returns the original molar_enthalpy", &
                        NON_ZERO_DOUBLE_PRECISION_PAIR_GENERATOR, &
                        check_multiply_divide) &
                , describe( &
                        "operator(==)", &
                        [ it( &
                                "is true for the same molar_enthalpy", &
                                DOUBLE_PRECISION_GENERATOR, &
                                check_equal_with_same_number) &
                        , it( &
                                "is false for different molar_enthalpys", &
                                DOUBLE_PRECISION_GENERATOR, &
                ...
                , describe( &
                        "operator(/=)", &
                        [ it( &
                                "is false for the same molar_enthalpy", &
                                DOUBLE_PRECISION_GENERATOR, &
                                check_not_equal_with_same_number) &
                        , it( &
                                "is true for different molar_enthalpys", &
                                DOUBLE_PRECISION_GENERATOR, &
                                check_not_equal_with_different_numbers) &
                        ]) &
                , describe( &
                        "%equal(molar_enthalpy, within)", &
                        [ it( &
                                "is true for the same molar_enthalpy even for tiny tolerance", &
                                DOUBLE_PRECISION_GENERATOR, &
    ...
    contains
        pure function check_round_trip_(input) result(result__)
            ...
            type(molar_enthalpy_t) :: intermediate
    ...
    pure function check_conversion_factors_are_inverse( &
        ...
        class(molar_enthalpy_unit_t), intent(in) :: to
        class(molar_enthalpy_unit_t), intent(in) :: from
        type(result_t) :: result_
    ...
    contains
        function do_check(input) result(result__)
            ...
            type(molar_enthalpy_t) :: original_molar_enthalpy
            type(fallible_molar_enthalpy_t) :: maybe_molar_enthalpy
            type(molar_enthalpy_t) :: new_molar_enthalpy

            select type (input)
            type is (double_precision_input_t)
                original_molar_enthalpy = input%input().unit.units
                maybe_molar_enthalpy = parse_molar_enthalpy( &
                        original_molar_enthalpy%to_string_in(units))
                new_molar_enthalpy = maybe_molar_enthalpy%molar_enthalpy()
                errors = maybe_molar_enthalpy%errors()
                result__ = &
                        assert_equals( &
                                original_molar_enthalpy, &
                                new_molar_enthalpy) &
    ...
    function check_bad_string() result(result_)
        ...
        maybe_molar_enthalpy = parse_molar_enthalpy("bad")
        errors = maybe_molar_enthalpy%errors()
    ...
    function check_bad_unit() result(result_)
        ...
        maybe_molar_enthalpy = parse_molar_enthalpy("1.0 J/molbad", [JOULES_PER_MOL])
        errors = maybe_molar_enthalpy%errors()
        result_ = assert_that(errors.hasType.PARSE_ERROR, errors%to_string())
    end function
    ...
    function check_bad_number() result(result_)
        ...
        maybe_molar_enthalpy = parse_molar_enthalpy("bad J/mol")
        errors = maybe_molar_enthalpy%errors()
        result_ = assert_that(errors.hasType.PARSE_ERROR, errors%to_string())
    end function
    ...
    pure function check_add_zero(input) result(result_)
        ...
        type(molar_enthalpy_t) :: zero

        select type(input)
        type is (double_precision_input_t)
            molar_enthalpy = input%input().unit.JOULES_PER_MOL
            zero = 0.0d0.unit.JOULES_PER_MOL
            result_ = assert_equals(molar_enthalpy, molar_enthalpy + zero)
        class default
            result_ = fail("Expected a double_precision_input_t")
        end select
    end function
    ...
    pure function check_add_subtract(input) result(result_)
        ...
        type(molar_enthalpy_t) :: molar_enthalpy1
        type(molar_enthalpy_t) :: molar_enthalpy2

        select type(input)
        type is (double_precision_pair_input_t)
            molar_enthalpy1 = input%first().unit.JOULES_PER_MOL
            molar_enthalpy2 = input%second_().unit.JOULES_PER_MOL
            result_ = assert_equals_within_relative( &
                    molar_enthalpy1, &
                    (molar_enthalpy1 + molar_enthalpy2) - molar_enthalpy2, &
                    1.0d-8, &
                    "molar_enthalpy1 = " // molar_enthalpy1%to_string() &
                    // ", molar_enthalpy2 = " // molar_enthalpy2%to_string())
        class default
    ...
    pure function check_multiply_by_one(input) result(result_)
        ...
        type is (double_precision_input_t)
            molar_enthalpy = input%input().unit.JOULES_PER_MOL
            result_ = assert_equals(molar_enthalpy, molar_enthalpy * 1.0d0)
        class default
            result_ = fail("Expected a double_precision_input_t")
        end select
    end function
    ...
    pure function check_multiply_by_zero(input) result(result_)
        ...
        type is (double_precision_input_t)
            molar_enthalpy = input%input().unit.JOULES_PER_MOL
            zero = 0.0d0.unit.JOULES_PER_MOL
            result_ = assert_equals(zero, molar_enthalpy * 0.0d0)
        class default
            result_ = fail("Expected a double_precision_input_t")
        end select
    end function
    ...
    pure function check_divide_by_self(input) result(result_)
        ...
        type is (double_precision_input_t)
            molar_enthalpy = input%input().unit.JOULES_PER_MOL
            result_ = assert_equals(1.0d0, molar_enthalpy / molar_enthalpy)
        class default
            result_ = fail("Expected a double_precision_input_t")
        end select
    end function
    ...
    pure function check_multiply_divide(input) result(result_)
        ...
        select type (input)
        type is (double_precision_pair_input_t)
            molar_enthalpy = input%first().unit.JOULES_PER_MOL
            result_ = assert_equals(molar_enthalpy, (molar_enthalpy * input%second_()) / input%second_())
        class default
            result_ = fail("Expected a double_precision_pair_input_t")
        end select
    end function
    ...
    pure function check_equal_with_same_number(input) result(result_)
        ...
        type(molar_enthalpy_t) :: the_molar_enthalpy

        select type (input)
        type is (double_precision_input_t)
            the_molar_enthalpy = input%input().unit.JOULES_PER_MOL
            result_ = assert_that( &
                    the_molar_enthalpy == the_molar_enthalpy, &
                    the_molar_enthalpy%to_string() // " == " // the_molar_enthalpy%to_string())
        class default
    ...
    pure function check_equal_with_different_numbers(input) result(result_)
        ...
        select type (input)
        type is (double_precision_input_t)
            molar_enthalpy1 = input%input().unit.JOULES_PER_MOL
            molar_enthalpy2 = (input%input() + 1.0d0).unit.JOULES_PER_MOL
            result_ = assert_not( &
                    molar_enthalpy1 == molar_enthalpy2, &
                    molar_enthalpy1%to_string() // " == " // molar_enthalpy2%to_string())
        class default
            result_ = fail("Expected to get a double_precision_input_t")
        end select
    end function
    ...
    pure function check_equal_within_with_same_number(input) result(result_)
        ...
        type(molar_enthalpy_t) :: the_molar_enthalpy
        type(molar_enthalpy_t) :: tolerance
        ...
        type is (double_precision_input_t)
            ...
            result_ = assert_that( &
                    the_molar_enthalpy%equal(the_molar_enthalpy, within = tolerance), &
                    "(" // the_molar_enthalpy%to_string() // ")%equal(" &
                        // the_molar_enthalpy%to_string() // ", within = " &
                        // tolerance%to_string() // ")")
        class default
            result_ = fail("Expected to get a double_precision_input_t")
        end select
    ...
    pure function check_equal_within_with_close_numbers(input) result(result_)
        ...
        type is (double_precision_input_t)
            molar_enthalpy1 = input%input().unit.JOULES_PER_MOL
            molar_enthalpy2 = (input%input() + 0.05d0).unit.JOULES_PER_MOL
            tolerance = 0.1d0.unit.JOULES_PER_MOL
            result_ = assert_that( &
                    molar_enthalpy1%equal(molar_enthalpy2, within = tolerance), &
                    "(" // molar_enthalpy1%to_string() // ")%equal(" &
                        // molar_enthalpy2%to_string() // ", within = " &
                        // tolerance%to_string() // ")")
        class default
            result_ = fail("Expected to get a double_precision_input_t")
        end select
    ...
    pure function check_equal_within_with_different_numbers(input) result(result_)
        ...
        type is (double_precision_input_t)
            molar_enthalpy1 = input%input().unit.JOULES_PER_MOL
            molar_enthalpy2 = (input%input() + 0.2d0).unit.JOULES_PER_MOL
            tolerance = 0.1d0.unit.JOULES_PER_MOL
            result_ = assert_not( &
                    molar_enthalpy1%equal(molar_enthalpy2, within = tolerance), &
                    "(" // molar_enthalpy1%to_string() // ")%equal(" &
    ...
    pure function check_greater_than_or_equal_with_same_numbers(input) result(result_)
        ...
        type is (double_precision_input_t)
            molar_enthalpy1 = input%input().unit.JOULES_PER_MOL
            molar_enthalpy2 = input%input().unit.JOULES_PER_MOL
            result_ = assert_that( &
                    molar_enthalpy1 >= molar_enthalpy2, &
                    molar_enthalpy1%to_string() // " >= " // molar_enthalpy2%to_string())
        class default
