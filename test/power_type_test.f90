module power_type_test
    ...
    type, public, extends(Input_t) :: UnitsInput_t
        class(PowerUnit_t), allocatable :: unit
    end type UnitsInput_t

    type, public, extends(Input_t) :: UnitsPairInput_t
        class(PowerUnit_t), allocatable :: first
        class(PowerUnit_t), allocatable :: second
...
contains
    function test_power() result(tests)
        ...
        examples = makeUnitsExamples(PROVIDED_POWER_UNITS)
        ...
        tests = describe("Power_t", individual_tests)
    ...
    pure function checkBadString() result(result_)
        ...
        call fromString("bad", errors, power)
    ...
    pure function checkBadNumber() result(result_)
        ...
        call fromString("bad W", errors, power)
        result_ = assertThat(errors.hasType.PARSE_ERROR, errors%toString())
    ...
    pure function checkConversionFactorsAreInverse( &
        ...
        class(PowerUnit_t), intent(in) :: to
        class(PowerUnit_t), intent(in) :: from
        type(Result_t) :: result_
    ...
    contains
        pure function doCheck(input) result(result__)
            ...
            select type (input)
            type is (DoublePrecisionInput_t)
                original_power = input%value_.unit.units
                call fromString( &
                        original_power%toStringIn(units), &
                        errors, &
                        new_power)
                result__ = &
                        assertEquals( &
