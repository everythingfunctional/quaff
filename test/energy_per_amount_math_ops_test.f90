module energy_per_amount_math_ops_test
...
contains
    function test_addition_subtraction_operators() result(tests)
        ...
        individual_tests(3) = It( &
                ...
                checkAddSubtract)
        tests = Describe("EnergyPerAmount_t (+/-) operators", individual_tests)
    ...
    function test_multiplication_division_operator() result(tests)
        ...
        individual_tests(1) = It( &
                "A energy_per_amount multiplied by 1 equals itself", &
                DOUBLE_PRECISION_GENERATOR, &
                checkMultiplyByOne)
        individual_tests(2) = It( &
                "A energy_per_amount multiplied by 0 is 0", &
                DOUBLE_PRECISION_GENERATOR, &
                checkMultiplyByZero)
        individual_tests(3) = It( &
                "A energy_per_amount divided by 1 equals itself", &
                DOUBLE_PRECISION_GENERATOR, &
                checkDivideByOne)
        individual_tests(4) = It( &
                "A energy_per_amount divided by itself equals 1", &
                NON_ZERO_DOUBLE_PRECISION_GENERATOR, &
                checkDivideBySelf)
        ...
        individual_tests(6) = It( &
                "Dividing and multiplying by the same number returns the original energy_per_amount", &
                NON_ZERO_DOUBLE_PRECISION_PAIR_GENERATOR, &
                checkDivideMultiply)
        tests = Describe("EnergyPerAmount_t (* & / ) operators", individual_tests)
    ...
    pure function checkAddZero(input) result(result_)
        ...
        type(EnergyPerAmount_t) :: zero

        select type(input)
        type is (DoublePrecisionInput_t)
            energy_per_amount = input%value_.unit.JOULES_PER_MOL
            zero = 0.0d0.unit.JOULES_PER_MOL
            result_ = assertEquals(energy_per_amount, energy_per_amount + zero)
        class default
    ...
    pure function checkAddSubtract(input) result(result_)
        ...
        type is (DoublePrecisionPairInput_t)
            ...
            energy_per_amount2 = input%second.unit.JOULES_PER_MOL
            result_ = assertEqualsWithinRelative( &
                    energy_per_amount1, &
                    (energy_per_amount1 + energy_per_amount2) - energy_per_amount2, &
                    1.0d-8, &
                    "energy_per_amount1 = " // energy_per_amount1%toString() &
                    // ", energy_per_amount2 = " // energy_per_amount2%toString())
        class default
    ...
    pure function checkMultiplyByOne(input) result(result_)
        ...
        type is (DoublePrecisionInput_t)
            energy_per_amount = input%value_.unit.JOULES_PER_MOL
            result_ = assertEquals(energy_per_amount, energy_per_amount * 1.0d0)
        class default
            result_ = fail("Expected a DoublePrecisionInput_t")
        end select
    ...
    pure function checkMultiplyByZero(input) result(result_)
        ...
        type is (DoublePrecisionInput_t)
            energy_per_amount = input%value_.unit.JOULES_PER_MOL
            zero = 0.0d0.unit.JOULES_PER_MOL
            result_ = assertEquals(zero, energy_per_amount * 0.0d0)
        class default
            result_ = fail("Expected a DoublePrecisionInput_t")
        end select
    ...
    pure function checkDivideBySelf(input) result(result_)
        ...
        type is (DoublePrecisionInput_t)
            energy_per_amount = input%value_.unit.JOULES_PER_MOL
            result_ = assertEquals(1.0d0, energy_per_amount / energy_per_amount)
        class default
            result_ = fail("Expected a DoublePrecisionInput_t")
        end select
    ...
    pure function checkMultiplyDivide(input) result(result_)
        ...
        type is (DoublePrecisionPairInput_t)
            ...
            result_ = assertEquals(energy_per_amount, energy_per_amount * input%second / input%second)
        class default
            result_ = fail("Expected a DoublePrecisionPairInput_t")
        end select
...
end module energy_per_amount_math_ops_test
