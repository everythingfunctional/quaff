module thermal_conductivity_type_test
    ...
    type, public, extends(Input_t) :: UnitsInput_t
        class(ThermalConductivityUnit_t), allocatable :: unit
    end type UnitsInput_t

    type, public, extends(Input_t) :: UnitsPairInput_t
        class(ThermalConductivityUnit_t), allocatable :: first
        class(ThermalConductivityUnit_t), allocatable :: second
...
contains
    ...
    pure function checkBadString() result(result_)
        ...
        call fromString("bad", errors, thermal_conductivity)
    ...
    pure function checkBadNumber() result(result_)
        ...
        call fromString("bad W/(m K)", errors, thermal_conductivity)
        result_ = assertThat(errors.hasType.PARSE_ERROR, errors%toString())
    ...
    contains
        pure function checkRoundTrip_(input) result(result__)
            ...
            type(ThermalConductivity_t) :: intermediate
    ...
    contains
        pure function doCheck(input) result(result__)
            ...
            type(ErrorList_t) :: errors
            type(ThermalConductivity_t) :: original_thermal_conductivity
            type(ThermalConductivity_t) :: new_thermal_conductivity

            select type (input)
            type is (DoublePrecisionInput_t)
                original_thermal_conductivity = input%value_.unit.units
                call fromString( &
                        original_thermal_conductivity%toStringIn(units), &
                        errors, &
...
end module thermal_conductivity_type_test
