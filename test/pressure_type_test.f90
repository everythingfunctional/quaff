module pressure_type_test
    ...
    type, public, extends(Input_t) :: UnitsInput_t
        class(PressureUnit_t), allocatable :: unit
    end type UnitsInput_t

    type, public, extends(Input_t) :: UnitsPairInput_t
        class(PressureUnit_t), allocatable :: first
        class(PressureUnit_t), allocatable :: second
...
contains
    function test_pressure() result(tests)
        ...
        examples = makeUnitsExamples(PROVIDED_PRESSURE_UNITS)
        ...
        tests = describe("Pressure_t", individual_tests)
    end function test_pressure
    ...
    pure function checkBadString() result(result_)
        ...
        call fromString("bad", errors, pressure)
    ...
    pure function checkBadUnit() result(result_)
        ...
        call fromString( &
                "1.0 bad", [PASCALS], errors, pressure)
        result_ = assertThat(errors.hasType.PARSE_ERROR, errors%toString())
    ...
    pure function checkBadNumber() result(result_)
        ...
        call fromString("bad Pa", errors, pressure)
        result_ = assertThat(errors.hasType.PARSE_ERROR, errors%toString())
    ...
    pure function checkConversionFactorsAreInverse( &
        ...
        class(PressureUnit_t), intent(in) :: to
        class(PressureUnit_t), intent(in) :: from
        type(Result_t) :: result_
    ...
    contains
        pure function doCheck(input) result(result__)
            ...
            select type (input)
            type is (DoublePrecisionInput_t)
                original_pressure = input%value_.unit.units
                call fromString( &
                        original_pressure%toStringIn(units), &
                        errors, &
                        new_pressure)
                result__ = &
                        assertEquals( &
