module burnup_type_test
    ...
    type, public, extends(Input_t) :: UnitsInput_t
        class(BurnupUnit_t), allocatable :: unit
    end type UnitsInput_t

    type, public, extends(Input_t) :: UnitsPairInput_t
        class(BurnupUnit_t), allocatable :: first
        class(BurnupUnit_t), allocatable :: second
...
contains
    function test_burnup() result(tests)
        ...
        examples = makeUnitsExamples(PROVIDED_BURNUP_UNITS)
        ...
        tests = describe("Burnup_t", individual_tests)
    end function test_burnup
    ...
    pure function checkBadString() result(result_)
        ...
        call fromString("bad", errors, burnup)
    ...
    pure function checkBadUnit() result(result_)
        ...
        call fromString( &
                "1.0 bad", [WATT_SECONDS_PER_KILOGRAM], errors, burnup)
        result_ = assertThat(errors.hasType.PARSE_ERROR, errors%toString())
    ...
    pure function checkBadNumber() result(result_)
        ...
        call fromString("bad (W s)/kg", errors, burnup)
        result_ = assertThat(errors.hasType.PARSE_ERROR, errors%toString())
    ...
    pure function checkConversionFactorsAreInverse( &
        ...
        class(BurnupUnit_t), intent(in) :: to
        class(BurnupUnit_t), intent(in) :: from
        type(Result_t) :: result_
    ...
    contains
        pure function doCheck(input) result(result__)
            ...
            select type (input)
            type is (DoublePrecisionInput_t)
                original_burnup = input%value_.unit.units
                call fromString( &
                        original_burnup%toStringIn(units), &
                        errors, &
                        new_burnup)
                result__ = &
                        assertEquals( &
