module energy_per_amount_test
    ...
    use energy_per_amount_utilities_m, only: &
...
contains
    function test_energy_per_amount() result(tests)
        ...
        examples = make_units_examples(PROVIDED_ENERGY_PER_AMOUNT_UNITS)
        tests = describe( &
                "energy_per_amount_t", &
                [ it( &
                ...
                , it( &
                        "adding zero returns the original energy_per_amount", &
                        DOUBLE_PRECISION_GENERATOR, &
                        check_add_zero) &
                , it( &
                        "subtracting zero returns the original energy_per_amount", &
                        DOUBLE_PRECISION_GENERATOR, &
                        check_subtract_zero) &
                , it( &
                        "adding and subtracting the same energy_per_amount returns the original energy_per_amount", &
                        DOUBLE_PRECISION_PAIR_GENERATOR, &
                        check_add_subtract) &
                , it( &
                        "multiplying by one returns the original energy_per_amount", &
                        DOUBLE_PRECISION_GENERATOR, &
                        check_multiply_by_one) &
                , it( &
                        "multiplying by zero returns zero energy_per_amount", &
                        DOUBLE_PRECISION_GENERATOR, &
                        check_multiply_by_zero) &
                , it( &
                        "dividing by one returns the original energy_per_amount", &
                        DOUBLE_PRECISION_GENERATOR, &
                        check_divide_by_one) &
                , it( &
                ...
                , it( &
                        "multiplying and dividing by the same number returns the original energy_per_amount", &
                        NON_ZERO_DOUBLE_PRECISION_PAIR_GENERATOR, &
                        check_multiply_divide) &
                , describe( &
                        "operator(==)", &
                        [ it( &
                                "is true for the same energy_per_amount", &
                                DOUBLE_PRECISION_GENERATOR, &
                                check_equal_with_same_number) &
                        , it( &
                                "is false for different energy_per_amounts", &
                                DOUBLE_PRECISION_GENERATOR, &
                ...
                , describe( &
                        "operator(/=)", &
                        [ it( &
                                "is false for the same energy_per_amount", &
                                DOUBLE_PRECISION_GENERATOR, &
                                check_not_equal_with_same_number) &
                        , it( &
                                "is true for different energy_per_amounts", &
                                DOUBLE_PRECISION_GENERATOR, &
                                check_not_equal_with_different_numbers) &
                        ]) &
                , describe( &
                        "%equal(energy_per_amount, within)", &
                        [ it( &
                                "is true for the same energy_per_amount even for tiny tolerance", &
                                DOUBLE_PRECISION_GENERATOR, &
    ...
    contains
        pure function check_round_trip_(input) result(result__)
            ...
            type(energy_per_amount_t) :: intermediate
    ...
    pure function check_conversion_factors_are_inverse( &
        ...
        class(energy_per_amount_unit_t), intent(in) :: to
        class(energy_per_amount_unit_t), intent(in) :: from
        type(result_t) :: result_
    ...
    contains
        function do_check(input) result(result__)
            ...
            type(energy_per_amount_t) :: original_energy_per_amount
            type(fallible_energy_per_amount_t) :: maybe_energy_per_amount
            type(energy_per_amount_t) :: new_energy_per_amount

            select type (input)
            type is (double_precision_input_t)
                original_energy_per_amount = input%input().unit.units
                maybe_energy_per_amount = parse_energy_per_amount( &
                        original_energy_per_amount%to_string_in(units))
                new_energy_per_amount = maybe_energy_per_amount%energy_per_amount()
                errors = maybe_energy_per_amount%errors()
                result__ = &
                        assert_equals( &
    ...
    function check_bad_string() result(result_)
        ...
        maybe_energy_per_amount = parse_energy_per_amount("bad")
        errors = maybe_energy_per_amount%errors()
    ...
    function check_bad_unit() result(result_)
        ...
        maybe_energy_per_amount = parse_energy_per_amount("1.0 J/molbad", [JOULES_PER_MOL])
        errors = maybe_energy_per_amount%errors()
        result_ = assert_that(errors.hasType.PARSE_ERROR, errors%to_string())
    end function
    ...
    function check_bad_number() result(result_)
        ...
        maybe_energy_per_amount = parse_energy_per_amount("bad J/mol")
        errors = maybe_energy_per_amount%errors()
        result_ = assert_that(errors.hasType.PARSE_ERROR, errors%to_string())
    end function
    ...
    pure function check_negation(input) result(result_)
        ...
        type is (double_precision_input_t)
            result_ = assert_equals( &
                    (-input%input()).unit.JOULES_PER_MOL, &
                    -(input%input().unit.JOULES_PER_MOL))
        class default
    ...
    pure function check_add_zero(input) result(result_)
        ...
        type(energy_per_amount_t) :: zero

        select type(input)
        type is (double_precision_input_t)
            energy_per_amount = input%input().unit.JOULES_PER_MOL
            zero = 0.0d0.unit.JOULES_PER_MOL
            result_ = assert_equals(energy_per_amount, energy_per_amount + zero)
        class default
            result_ = fail("Expected a double_precision_input_t")
        end select
    end function
    ...
    pure function check_add_subtract(input) result(result_)
        ...
        type(energy_per_amount_t) :: energy_per_amount1
        type(energy_per_amount_t) :: energy_per_amount2

        select type(input)
        type is (double_precision_pair_input_t)
            energy_per_amount1 = input%first().unit.JOULES_PER_MOL
            energy_per_amount2 = input%second_().unit.JOULES_PER_MOL
            result_ = assert_equals_within_relative( &
                    energy_per_amount1, &
                    (energy_per_amount1 + energy_per_amount2) - energy_per_amount2, &
                    1.0d-8, &
                    "energy_per_amount1 = " // energy_per_amount1%to_string() &
                    // ", energy_per_amount2 = " // energy_per_amount2%to_string())
        class default
    ...
    pure function check_multiply_by_one(input) result(result_)
        ...
        type is (double_precision_input_t)
            energy_per_amount = input%input().unit.JOULES_PER_MOL
            result_ = assert_equals(energy_per_amount, energy_per_amount * 1.0d0)
        class default
            result_ = fail("Expected a double_precision_input_t")
        end select
    end function
    ...
    pure function check_multiply_by_zero(input) result(result_)
        ...
        type is (double_precision_input_t)
            energy_per_amount = input%input().unit.JOULES_PER_MOL
            zero = 0.0d0.unit.JOULES_PER_MOL
            result_ = assert_equals(zero, energy_per_amount * 0.0d0)
        class default
            result_ = fail("Expected a double_precision_input_t")
        end select
    end function
    ...
    pure function check_divide_by_self(input) result(result_)
        ...
        type is (double_precision_input_t)
            energy_per_amount = input%input().unit.JOULES_PER_MOL
            result_ = assert_equals(1.0d0, energy_per_amount / energy_per_amount)
        class default
            result_ = fail("Expected a double_precision_input_t")
        end select
    end function
    ...
    pure function check_multiply_divide(input) result(result_)
        ...
        select type (input)
        type is (double_precision_pair_input_t)
            energy_per_amount = input%first().unit.JOULES_PER_MOL
            result_ = assert_equals(energy_per_amount, (energy_per_amount * input%second_()) / input%second_())
        class default
            result_ = fail("Expected a double_precision_pair_input_t")
        end select
    end function
    ...
    pure function check_equal_with_same_number(input) result(result_)
        ...
        type(energy_per_amount_t) :: the_energy_per_amount

        select type (input)
        type is (double_precision_input_t)
            the_energy_per_amount = input%input().unit.JOULES_PER_MOL
            result_ = assert_that( &
                    the_energy_per_amount == the_energy_per_amount, &
                    the_energy_per_amount%to_string() // " == " // the_energy_per_amount%to_string())
        class default
    ...
    pure function check_equal_with_different_numbers(input) result(result_)
        ...
        select type (input)
        type is (double_precision_input_t)
            energy_per_amount1 = input%input().unit.JOULES_PER_MOL
            energy_per_amount2 = (input%input() + 1.0d0).unit.JOULES_PER_MOL
            result_ = assert_not( &
                    energy_per_amount1 == energy_per_amount2, &
                    energy_per_amount1%to_string() // " == " // energy_per_amount2%to_string())
        class default
            result_ = fail("Expected to get a double_precision_input_t")
        end select
    end function
    ...
    pure function check_equal_within_with_same_number(input) result(result_)
        ...
        type(energy_per_amount_t) :: the_energy_per_amount
        type(energy_per_amount_t) :: tolerance
        ...
        type is (double_precision_input_t)
            ...
            result_ = assert_that( &
                    ...
                    "(" // the_energy_per_amount%to_string() // ")%equal(" &
                        // the_energy_per_amount%to_string() // ", within = " &
                        // tolerance%to_string() // ")")
        class default
            result_ = fail("Expected to get a double_precision_input_t")
        end select
    ...
    pure function check_equal_within_with_close_numbers(input) result(result_)
        ...
        type is (double_precision_input_t)
            energy_per_amount1 = input%input().unit.JOULES_PER_MOL
            energy_per_amount2 = (input%input() + 0.05d0).unit.JOULES_PER_MOL
            tolerance = 0.1d0.unit.JOULES_PER_MOL
            result_ = assert_that( &
                    energy_per_amount1%equal(energy_per_amount2, within = tolerance), &
                    "(" // energy_per_amount1%to_string() // ")%equal(" &
                        // energy_per_amount2%to_string() // ", within = " &
                        // tolerance%to_string() // ")")
        class default
            result_ = fail("Expected to get a double_precision_input_t")
        end select
    ...
    pure function check_equal_within_with_different_numbers(input) result(result_)
        ...
        type is (double_precision_input_t)
            energy_per_amount1 = input%input().unit.JOULES_PER_MOL
            energy_per_amount2 = (input%input() + 0.2d0).unit.JOULES_PER_MOL
            tolerance = 0.1d0.unit.JOULES_PER_MOL
            result_ = assert_not( &
                    energy_per_amount1%equal(energy_per_amount2, within = tolerance), &
                    "(" // energy_per_amount1%to_string() // ")%equal(" &
    ...
    pure function check_greater_than_or_equal_with_same_numbers(input) result(result_)
        ...
        type is (double_precision_input_t)
            energy_per_amount1 = input%input().unit.JOULES_PER_MOL
            energy_per_amount2 = input%input().unit.JOULES_PER_MOL
            result_ = assert_that( &
                    energy_per_amount1 >= energy_per_amount2, &
                    energy_per_amount1%to_string() // " >= " // energy_per_amount2%to_string())
        class default
