module enthalpy_type_test
    ...
    type, public, extends(Input_t) :: UnitsInput_t
        class(EnthalpyUnit_t), allocatable :: unit
    end type UnitsInput_t

    type, public, extends(Input_t) :: UnitsPairInput_t
        class(EnthalpyUnit_t), allocatable :: first
        class(EnthalpyUnit_t), allocatable :: second
...
contains
    function test_enthalpy() result(tests)
        ...
        examples = makeUnitsExamples(PROVIDED_ENTHALPY_UNITS)
        ...
        tests = describe("Enthalpy_t", individual_tests)
    end function test_enthalpy
    ...
    pure function checkBadString() result(result_)
        ...
        call fromString("bad", errors, enthalpy)
    ...
    pure function checkBadUnit() result(result_)
        ...
        call fromString( &
                "1.0 bad", [JOULES_PER_KILOGRAM], errors, enthalpy)
        result_ = assertThat(errors.hasType.PARSE_ERROR, errors%toString())
    ...
    pure function checkBadNumber() result(result_)
        ...
        call fromString("bad J/kg", errors, enthalpy)
        result_ = assertThat(errors.hasType.PARSE_ERROR, errors%toString())
    ...
    pure function checkConversionFactorsAreInverse( &
        ...
        class(EnthalpyUnit_t), intent(in) :: to
        class(EnthalpyUnit_t), intent(in) :: from
        type(Result_t) :: result_
    ...
    contains
        pure function doCheck(input) result(result__)
            ...
            select type (input)
            type is (DoublePrecisionInput_t)
                original_enthalpy = input%value_.unit.units
                call fromString( &
                        original_enthalpy%toStringIn(units), &
                        errors, &
                        new_enthalpy)
                result__ = &
                        assertEquals( &
