module amount_utilities_m
    use quaff, only: amount_unit_t
    ...
    type, extends(input_t) :: units_pair_input_t
        private
        class(amount_unit_t), allocatable :: first_
        class(amount_unit_t), allocatable :: second__
    contains
        private
...
contains
    ...
    function units_pair_input_constructor(first, second_) result(units_pair_input)
        class(amount_unit_t), intent(in) :: first
        class(amount_unit_t), intent(in) :: second_
    ...
    function first(self)
        ...
        class(amount_unit_t), allocatable :: first
    ...
    function second_(self)
        class(units_pair_input_t), intent(in) :: self
        class(amount_unit_t), allocatable :: second_
