module amount_rate_test
    ...
    use amount_rate_utilities_m, only: &
    ...
    implicit none
    private
    public :: test_amount_rate
contains
    function test_amount_rate() result(tests)
        type(test_item_t) :: tests

        type(units_examples_t) :: examples

        examples = make_units_examples(PROVIDED_AMOUNT_RATE_UNITS)
        tests = describe( &
                "amount_rate_t", &
                [ it( &
                ...
                , it( &
                        ...
                        check_add_zero) &
                , it( &
                        "subtracting zero returns the original amount_rate", &
                        DOUBLE_PRECISION_GENERATOR, &
                        check_subtract_zero) &
                , it( &
                        "adding and subtracting the same amount_rate returns the original amount_rate", &
                        DOUBLE_PRECISION_PAIR_GENERATOR, &
                        check_add_subtract) &
                , it( &
                        "multiplying by one returns the original amount_rate", &
                        DOUBLE_PRECISION_GENERATOR, &
                        check_multiply_by_one) &
                , it( &
                        "multiplying by zero returns zero amount_rate", &
                        DOUBLE_PRECISION_GENERATOR, &
                        check_multiply_by_zero) &
                , it( &
                        "dividing by one returns the original amount_rate", &
                        DOUBLE_PRECISION_GENERATOR, &
                        check_divide_by_one) &
                , it( &
                ...
                , it( &
                        "multiplying and dividing by the same number returns the original amount_rate", &
                        NON_ZERO_DOUBLE_PRECISION_PAIR_GENERATOR, &
                        check_multiply_divide) &
                , describe( &
                        "operator(==)", &
                        [ it( &
                                "is true for the same amount_rate", &
                                DOUBLE_PRECISION_GENERATOR, &
                                check_equal_with_same_number) &
                        , it( &
                                "is false for different amount_rates", &
                                DOUBLE_PRECISION_GENERATOR, &
                ...
                , describe( &
                        "operator(/=)", &
                        [ it( &
                                "is false for the same amount_rate", &
                                DOUBLE_PRECISION_GENERATOR, &
                                check_not_equal_with_same_number) &
                        , it( &
                                "is true for different amount_rates", &
                                DOUBLE_PRECISION_GENERATOR, &
                                check_not_equal_with_different_numbers) &
                        ]) &
                , describe( &
                        "%equal(amount_rate, within)", &
                        [ it( &
                                "is true for the same amount_rate even for tiny tolerance", &
                                DOUBLE_PRECISION_GENERATOR, &
    ...
    contains
        pure function check_round_trip_(input) result(result__)
            ...
            type(amount_rate_t) :: intermediate
    ...
    pure function check_conversion_factors_are_inverse( &
        ...
        class(amount_rate_unit_t), intent(in) :: to
        class(amount_rate_unit_t), intent(in) :: from
        type(result_t) :: result_
    ...
    contains
        function do_check(input) result(result__)
            ...
            type(amount_rate_t) :: original_amount_rate
            type(fallible_amount_rate_t) :: maybe_amount_rate
            type(amount_rate_t) :: new_amount_rate

            select type (input)
            type is (double_precision_input_t)
                original_amount_rate = input%input().unit.units
                maybe_amount_rate = parse_amount_rate( &
                        original_amount_rate%to_string_in(units))
                new_amount_rate = maybe_amount_rate%amount_rate()
                errors = maybe_amount_rate%errors()
                result__ = &
                        assert_equals( &
                                original_amount_rate, &
                                new_amount_rate) &
    ...
    function check_bad_string() result(result_)
        ...
        maybe_amount_rate = parse_amount_rate("bad")
        errors = maybe_amount_rate%errors()
    ...
    function check_bad_unit() result(result_)
        ...
        maybe_amount_rate = parse_amount_rate("1.0 mol/sbad", [MOLS_PER_SECOND])
        errors = maybe_amount_rate%errors()
        result_ = assert_that(errors.hasType.PARSE_ERROR, errors%to_string())
    end function
    ...
    function check_bad_number() result(result_)
        ...
        maybe_amount_rate = parse_amount_rate("bad mol/s")
        errors = maybe_amount_rate%errors()
        result_ = assert_that(errors.hasType.PARSE_ERROR, errors%to_string())
    end function
    ...
    pure function check_add_zero(input) result(result_)
        ...
        type(amount_rate_t) :: zero

        select type(input)
        type is (double_precision_input_t)
            amount_rate = input%input().unit.MOLS_PER_SECOND
            zero = 0.0d0.unit.MOLS_PER_SECOND
            result_ = assert_equals(amount_rate, amount_rate + zero)
        class default
    ...
    pure function check_add_subtract(input) result(result_)
        ...
        type(amount_rate_t) :: amount_rate1
        type(amount_rate_t) :: amount_rate2

        select type(input)
        type is (double_precision_pair_input_t)
            amount_rate1 = input%first().unit.MOLS_PER_SECOND
            amount_rate2 = input%second_().unit.MOLS_PER_SECOND
            result_ = assert_equals_within_relative( &
                    amount_rate1, &
                    (amount_rate1 + amount_rate2) - amount_rate2, &
                    1.0d-8, &
                    "amount_rate1 = " // amount_rate1%to_string() &
                    // ", amount_rate2 = " // amount_rate2%to_string())
        class default
    ...
    pure function check_multiply_by_one(input) result(result_)
        ...
        type is (double_precision_input_t)
            amount_rate = input%input().unit.MOLS_PER_SECOND
            result_ = assert_equals(amount_rate, amount_rate * 1.0d0)
        class default
            result_ = fail("Expected a double_precision_input_t")
        end select
    end function
    ...
    pure function check_multiply_by_zero(input) result(result_)
        ...
        type is (double_precision_input_t)
            amount_rate = input%input().unit.MOLS_PER_SECOND
            zero = 0.0d0.unit.MOLS_PER_SECOND
            result_ = assert_equals(zero, amount_rate * 0.0d0)
        class default
            result_ = fail("Expected a double_precision_input_t")
        end select
    end function
    ...
    pure function check_divide_by_self(input) result(result_)
        ...
        type is (double_precision_input_t)
            amount_rate = input%input().unit.MOLS_PER_SECOND
            result_ = assert_equals(1.0d0, amount_rate / amount_rate)
        class default
            result_ = fail("Expected a double_precision_input_t")
        end select
    end function
    ...
    pure function check_multiply_divide(input) result(result_)
        ...
        select type (input)
        type is (double_precision_pair_input_t)
            amount_rate = input%first().unit.MOLS_PER_SECOND
            result_ = assert_equals(amount_rate, (amount_rate * input%second_()) / input%second_())
        class default
            result_ = fail("Expected a double_precision_pair_input_t")
        end select
    end function
    ...
    pure function check_equal_with_same_number(input) result(result_)
        ...
        type(amount_rate_t) :: the_amount_rate

        select type (input)
        type is (double_precision_input_t)
            the_amount_rate = input%input().unit.MOLS_PER_SECOND
            result_ = assert_that( &
                    the_amount_rate == the_amount_rate, &
                    the_amount_rate%to_string() // " == " // the_amount_rate%to_string())
        class default
    ...
    pure function check_equal_with_different_numbers(input) result(result_)
        ...
        select type (input)
        type is (double_precision_input_t)
            amount_rate1 = input%input().unit.MOLS_PER_SECOND
            amount_rate2 = (input%input() + 1.0d0).unit.MOLS_PER_SECOND
            result_ = assert_not( &
                    amount_rate1 == amount_rate2, &
                    amount_rate1%to_string() // " == " // amount_rate2%to_string())
        class default
            result_ = fail("Expected to get a double_precision_input_t")
        end select
    end function
    ...
    pure function check_equal_within_with_same_number(input) result(result_)
        ...
        type(amount_rate_t) :: the_amount_rate
        type(amount_rate_t) :: tolerance

        select type (input)
        type is (double_precision_input_t)
            the_amount_rate = input%input().unit.MOLS_PER_SECOND
            tolerance = tiny(1.0d0).unit.MOLS_PER_SECOND
            result_ = assert_that( &
                    the_amount_rate%equal(the_amount_rate, within = tolerance), &
                    "(" // the_amount_rate%to_string() // ")%equal(" &
                        // the_amount_rate%to_string() // ", within = " &
                        // tolerance%to_string() // ")")
        class default
            result_ = fail("Expected to get a double_precision_input_t")
        end select
    ...
    pure function check_equal_within_with_close_numbers(input) result(result_)
        ...
        type is (double_precision_input_t)
            amount_rate1 = input%input().unit.MOLS_PER_SECOND
            amount_rate2 = (input%input() + 0.05d0).unit.MOLS_PER_SECOND
            tolerance = 0.1d0.unit.MOLS_PER_SECOND
            result_ = assert_that( &
                    amount_rate1%equal(amount_rate2, within = tolerance), &
                    "(" // amount_rate1%to_string() // ")%equal(" &
                        // amount_rate2%to_string() // ", within = " &
                        // tolerance%to_string() // ")")
        class default
            result_ = fail("Expected to get a double_precision_input_t")
        end select
    ...
    pure function check_equal_within_with_different_numbers(input) result(result_)
        ...
        type is (double_precision_input_t)
            amount_rate1 = input%input().unit.MOLS_PER_SECOND
            amount_rate2 = (input%input() + 0.2d0).unit.MOLS_PER_SECOND
            tolerance = 0.1d0.unit.MOLS_PER_SECOND
            result_ = assert_not( &
                    amount_rate1%equal(amount_rate2, within = tolerance), &
                    "(" // amount_rate1%to_string() // ")%equal(" &
    ...
    pure function check_greater_than_or_equal_with_same_numbers(input) result(result_)
        ...
        type is (double_precision_input_t)
            amount_rate1 = input%input().unit.MOLS_PER_SECOND
            amount_rate2 = input%input().unit.MOLS_PER_SECOND
            result_ = assert_that( &
                    amount_rate1 >= amount_rate2, &
                    amount_rate1%to_string() // " >= " // amount_rate2%to_string())
        class default
