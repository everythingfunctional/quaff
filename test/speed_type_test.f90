module speed_type_test
    ...
    type, public, extends(Input_t) :: UnitsInput_t
        class(SpeedUnit_t), allocatable :: unit
    end type UnitsInput_t

    type, public, extends(Input_t) :: UnitsPairInput_t
        class(SpeedUnit_t), allocatable :: first
        class(SpeedUnit_t), allocatable :: second
...
contains
    function test_speed() result(tests)
        ...
        examples = makeUnitsExamples(PROVIDED_SPEED_UNITS)
        ...
        tests = describe("Speed_t", individual_tests)
    ...
    pure function checkBadString() result(result_)
        ...
        call fromString("bad", errors, speed)
    ...
    pure function checkBadUnit() result(result_)
        ...
        call fromString( &
                "1.0 bad", [METERS_PER_SECOND], errors, speed)
        result_ = assertThat(errors.hasType.PARSE_ERROR, errors%toString())
    ...
    pure function checkBadNumber() result(result_)
        ...
        call fromString("bad m/s", errors, speed)
        result_ = assertThat(errors.hasType.PARSE_ERROR, errors%toString())
    ...
    pure function checkConversionFactorsAreInverse( &
        ...
        class(SpeedUnit_t), intent(in) :: to
        class(SpeedUnit_t), intent(in) :: from
        type(Result_t) :: result_
    ...
    contains
        pure function doCheck(input) result(result__)
            ...
            select type (input)
            type is (DoublePrecisionInput_t)
                original_speed = input%value_.unit.units
                call fromString( &
                        original_speed%toStringIn(units), &
                        errors, &
                        new_speed)
                result__ = &
                        assertEquals( &
