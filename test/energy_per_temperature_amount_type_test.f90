module energy_per_temperature_amount_type_test
    ...
    type, public, extends(Input_t) :: UnitsInput_t
        class(EnergyPerTemperatureAmountUnit_t), allocatable :: unit
    end type UnitsInput_t

    type, public, extends(Input_t) :: UnitsPairInput_t
        class(EnergyPerTemperatureAmountUnit_t), allocatable :: first
        class(EnergyPerTemperatureAmountUnit_t), allocatable :: second
...
contains
    function test_energy_per_temperature_amount() result(tests)
        ...
        examples = makeUnitsExamples(PROVIDED_ENERGY_PER_TEMPERATURE_AMOUNT_UNITS)
        ...
        tests = describe("EnergyPerTemperatureAmount_t", individual_tests)
    end function test_energy_per_temperature_amount
    ...
    pure function checkBadString() result(result_)
        ...
        call fromString("bad", errors, energy_per_temperature_amount)
    ...
    pure function checkBadUnit() result(result_)
        ...
        call fromString( &
                "1.0 bad", [JOULES_PER_KELVIN_MOL], errors, energy_per_temperature_amount)
        result_ = assertThat(errors.hasType.PARSE_ERROR, errors%toString())
    ...
    pure function checkBadNumber() result(result_)
        ...
        call fromString("bad J/(K mol)", errors, energy_per_temperature_amount)
        result_ = assertThat(errors.hasType.PARSE_ERROR, errors%toString())
    ...
    contains
        pure function checkRoundTrip_(input) result(result__)
            ...
            type(EnergyPerTemperatureAmount_t) :: intermediate
    ...
    pure function checkConversionFactorsAreInverse( &
        ...
        class(EnergyPerTemperatureAmountUnit_t), intent(in) :: to
        class(EnergyPerTemperatureAmountUnit_t), intent(in) :: from
        type(Result_t) :: result_
    ...
    contains
        pure function doCheck(input) result(result__)
            ...
            type(ErrorList_t) :: errors
            type(EnergyPerTemperatureAmount_t) :: original_energy_per_temperature_amount
            type(EnergyPerTemperatureAmount_t) :: new_energy_per_temperature_amount

            select type (input)
            type is (DoublePrecisionInput_t)
                original_energy_per_temperature_amount = input%value_.unit.units
                call fromString( &
                        original_energy_per_temperature_amount%toStringIn(units), &
                        errors, &
...
end module energy_per_temperature_amount_type_test
