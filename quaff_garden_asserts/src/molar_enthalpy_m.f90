module molar_enthalpy_asserts_m
    use iso_varying_string, only: varying_string, operator(//), var_str
    use quaff, only: molar_enthalpy_t
...
contains
    pure function assert_equals_basic(expected, actual) result(result_)
        type(molar_enthalpy_t), intent(in) :: expected
        type(molar_enthalpy_t), intent(in) :: actual
    ...
    pure function assert_equals_within_absolute_basic( &
        ...
        type(molar_enthalpy_t), intent(in) :: expected
        type(molar_enthalpy_t), intent(in) :: actual
        type(molar_enthalpy_t), intent(in) :: tolerance
        type(result_t) :: result_
