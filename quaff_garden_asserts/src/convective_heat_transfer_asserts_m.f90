module convective_heat_transfer_asserts_m
    use iso_varying_string, only: varying_string, operator(//), var_str
    use quaff, only: convective_heat_transfer_t
...
contains
    pure function assert_equals_basic(expected, actual) result(result_)
        type(convective_heat_transfer_t), intent(in) :: expected
        type(convective_heat_transfer_t), intent(in) :: actual
    ...
    pure function assert_equals_within_absolute_basic( &
        ...
        type(convective_heat_transfer_t), intent(in) :: expected
        type(convective_heat_transfer_t), intent(in) :: actual
        type(convective_heat_transfer_t), intent(in) :: tolerance
        type(result_t) :: result_
