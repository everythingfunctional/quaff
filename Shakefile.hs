...
main = do
  ...
  let testCollectionModules =
        Set.fromList $ map (takeFileName . dropExtension) testCollectionFiles
  ...
  let (projectFilesMap, projectDependsMap, projectCommandsMap) =
        makeSourceRuleMaps compilerFlags projectSearchTree (Set.toList sources)
  let (testFilesMap, testDependsMap, testCommandsMap) =
        makeSourceRuleMaps compilerFlags testSearchTree (Set.toList testSources)
  ...
  shakeArgs shakeOptions { shakeFiles    = shakeDir
    ...
    $ do
        (`Map.lookup` filesMap) &?> \(obj : _) -> do
          need =<< liftIO (fromMaybe undefined (Map.lookup obj dependsMap))
...
scanSourceFiles previousModules = foldl
 ...
 where
  addNextContents previousContents file = do
    ...
    case newContents of
      Left (source, program) -> return
        (source `Set.insert` sources, modules, program `Set.insert` programs)
...
scanSourceFile previousModules file = do
  ...
  let eitherContents =
        foldl addLineContents (Right (Set.empty, Set.empty, Nothing)) fileLines
  case eitherContents of
    Right contents -> case contents of
      (modulesContained, modulesUsed, Nothing) -> return
        $ Right (buildOutputWithModules file modulesUsed modulesContained)
      (modulesContained, modulesUsed, Just programName) -> return $ Left
      ...
      case parseFortranLine line of
        ...
        ModuleDefined moduleName ->
          case Map.lookup moduleName previousModules of
            ...
            Nothing -> if moduleName `Set.member` previousModulesContained
              then Left $ "module " ++ moduleName ++ " defined twice"
              else Right
                ( moduleName `Set.insert` previousModulesContained
                , previousModulesUsed
                , maybeProgram
                )
        ProgramDefined programName -> case maybeProgram of
          Just programName' ->
            Left
              ...
              ++ programName
          Nothing -> Right
            (previousModulesContained, previousModulesUsed, Just programName)
        Other ->
          Right (previousModulesContained, previousModulesUsed, maybeProgram)
    Left err -> Left err
...
makeSpecialSourceRuleMaps specialCommands flags moduleSearchTree sources =
  foldl
    ...
    (map (makeSpecialSourceRuleMap specialCommands flags moduleSearchTree)
         sources
    )
...
makeSpecialSourceRuleMap specialCommands flags moduleSearchTree source@(SourceFile src _ modulesContained)
  = let buildDir       = getNodeDirectory moduleSearchTree
        ...
        moduleFileNames =
            map ((buildDir </>) . (-<.> "mod")) $ Set.toList modulesContained
        additionalIncludeFlags =
            map ("-I" ++) (getAdditionalBuildDirs moduleSearchTree)
        filesMap = foldl
          (\previousMap newFile ->
            Map.insert newFile (objectFileName : moduleFileNames) previousMap
          )
          Map.empty
          (objectFileName : moduleFileNames)
        dependsMap = Map.singleton objectFileName compDeps
        commandMap =
            Map.singleton objectFileName $ case Map.lookup src specialCommands of
              Just command ->
...
makeExecutableMap moduleSearchTree program =
  ...
  in  (Set.singleton executable, Map.singleton executable linkDeps)
   ...
   where
    collect previousModules nextModule =
      if nextModule `Map.member` getNodeModules moduleSearchTree
        then
          return
          $ (getNodeDirectory moduleSearchTree </> nextModule <.> "mod")
